...
基于 **Java 25 + Spring Boot 4 + Spring Cloud 2025** 的企业级框架，以 Monorepo 形式提供 **96 个 Spring Boot Starter** 和 **12 个 Spring Cloud Starter**，覆盖数据访问、缓存、安全、消息、可观测、分布式系统、测试等领域。
...
| `kuma-boot-starter-data-migration` | 数据库迁移支持 |
| `kuma-boot-starter-data-vector` | 向量数据库集成 |
...
| `kuma-boot-starter-security-satoken` | Sa-Token 安全自动配置 |
| `kuma-boot-starter-totp` | TOTP 双因素认证（Google Authenticator 兼容）|
| `kuma-boot-starter-captcha` | 验证码（图形、滑块、点击）|
| `kuma-boot-starter-session` | 分布式会话支持 |
| `kuma-boot-starter-sign` | 请求签名与验签 |
...
| `kuma-boot-starter-mq-rabbitmq` | RabbitMQ 集成 |
| `kuma-boot-starter-mq-rocketmq` | RocketMQ 集成 |
...
| `kuma-boot-starter-audit-log` | 应用审计日志 |
| `kuma-boot-starter-otel` | OpenTelemetry 集成 |
...
| Starter | 说明 |
|---------|------|
| `kuma-boot-starter-arthas` | Arthas 诊断集成 |
| `kuma-boot-starter-jprofiler` | JProfiler 集成工具 |
| `kuma-boot-starter-jmeter` | 嵌入式 Apache JMeter 性能测试 |
| `kuma-boot-starter-jacoco` | JaCoCo 覆盖率工具 |
...
| `kuma-boot-starter-job-quartz` | Quartz 调度集成 |
| `kuma-boot-starter-job-powerjob` | PowerJob Worker 集成 |
| `kuma-boot-starter-job-xxl` | XXL-Job 执行器自动配置 |
| `kuma-boot-starter-oss-common` | 对象存储抽象接口 |
| `kuma-boot-starter-oss-minio` | MinIO 存储实现 |
| `kuma-boot-starter-oss-aliyun` | 阿里云 OSS 实现 |
| `kuma-boot-starter-oss-cos` | 腾讯云 COS 实现 |
| `kuma-boot-starter-oss-qiniu` | 七牛云对象存储实现 |
| `kuma-boot-starter-file-transfer` | 文件传输能力 |
...
| `kuma-cloud-starter-openfeign` | Spring Cloud OpenFeign 声明式 HTTP 客户端 |
| `kuma-cloud-starter-dubbo` | Apache Dubbo RPC 集成 |
...
|------|------|
| JDK | 25（强制）|
| Gradle | 9.6.1（通过 Wrapper 自动下载，无需手动安装）|
| MySQL | 8.0+ |
| Redis | 7.x |
