...
package com.kuma.boot.data.mybatis.mybatisplus.interceptor.datascope.dataPermission.db;
 ...
 *
 * <p>主要的SQL重写方法可见 {@link #builderExpression(Expression, Table)} 方法 参考 {@link
 * com.baomidou.mybatisplus.extension.plugins.inner.TenantLineInnerInterceptor} 实现。
 */
public class DataPermissionDatabaseInterceptor extends JsqlParserSupport
     ...
     * 处理where条件内的子查询
     *
     * <p>支持如下: 1. in 2. = 3. > 4. < 5. >= 6. <= 7. <> 8. EXISTS 9. NOT EXISTS
     *
     * <p>前提条件: 1. 子查询必须放在小括号中 2. 子查询一般放在比较操作符的右边
     *
    ...
    protected void processWhereSubSelect(Expression where) {
        ...
        if (where.toString().indexOf("SELECT") > 0) {
            ...
            if (where instanceof BinaryExpression expression) {
                ...
                // in
                // ItemsList itemsList = expression.getRightExpression();
    ...
    protected void processJoin(Join join) {
        if (join.getRightItem() instanceof Table fromTable) {
            ...
            join.setOnExpressions(ListUtil.of(originOnExpression));
        }
    }
    ...
    protected Expression builderExpression(Expression currentExpression, Table table) {
        ...
        if (currentExpression instanceof OrExpression) {
            // todo 需要适配jsqlparser 5.1
            // return new AndExpression(new Parenthesis().withExpression(currentExpression),
            // equalsTo);
        }
    ...
    /**
     * {@link MappedStatement} 缓存 目前主要用于，记录 {@link DataPermissionRule} 是否对指定 {@link MappedStatement}
     * 无效 如果无效，则可以避免 SQL 的解析，加快速度
     */
    static final class MappedStatementCache {
         ...
         *
         * <p>value：{@link MappedStatement#getId()} 编号
         */
        private final Map<Class<? extends DataPermissionRule>, Set<String>>
                noRewritableMappedStatements = new ConcurrentHashMap<>();

        public Map<Class<? extends DataPermissionRule>, Set<String>>
        getNoRewritableMappedStatements() {
        ...
        public void addNoRewritable(MappedStatement ms, List<DataPermissionRule> rules) {
            for (DataPermissionRule rule : rules) {
                ...
                if (CollUtil.isNotEmpty(mappedStatementIds)) {
                    ...
                    noRewritableMappedStatements.put(
                            rule.getClass(), new HashSet<>(Collections.singletonList(ms.getId())));
                }
            }
        }
    }
