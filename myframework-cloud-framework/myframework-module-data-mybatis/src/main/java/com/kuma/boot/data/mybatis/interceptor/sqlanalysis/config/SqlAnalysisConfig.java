...
import com.kuma.boot.data.mybatis.interceptor.sqlanalysis.analysis.SqlAnalysisSqlTypeEnum;
...
public class SqlAnalysisConfig {
    ...
    /**
     * 评分规则加载类， 默认
     * com.kuma.boot.data.mybatis.interceptor.sqlanalysis.rule.SqlScoreRuleLoaderDefault
     */
    private static String scoreRuleLoadClass;

    /**
     * 分析结果输出类，默认日志模式
     * com.kuma.boot.data.mybatis.interceptor.sqlanalysis.export.SqlScoreResultOutServiceDefault
     */
    ...
    public static void init(Properties properties) {
        try {
            ...
            if (StringUtils.isNotBlank(properties.getProperty("sqlReplaceModelSwitch"))) {
                sqlReplaceModelSwitch =
                        Boolean.valueOf(properties.getProperty("sqlReplaceModelSwitch"));
            }
            ...
            // 初始化ducc配置
            if (sqlReplaceModelSwitch != null
                    && sqlReplaceModelSwitch
                    && StringUtils.isNotBlank(properties.getProperty("duccAppName"))
                    && StringUtils.isNotBlank(properties.getProperty("duccUri"))
                    && StringUtils.isNotBlank(properties.getProperty("duccMonitorKey"))) {
                DuccMonitorUtil.start(
                        properties.getProperty("duccAppName"),
                        properties.getProperty("duccUri"),
                        properties.getProperty("duccMonitorKey"));
            }
