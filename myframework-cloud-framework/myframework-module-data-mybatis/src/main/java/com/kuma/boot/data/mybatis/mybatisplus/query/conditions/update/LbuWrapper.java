...
package com.kuma.boot.data.mybatis.mybatisplus.query.conditions.update;
 ...
 * 修改构造器
 *
 * <p>1, 对nested、eq、ne、gt、ge、lt、le、in、*like*、 等方法 进行条件判断，null 或 "" 字段不加入查询 2，对*like*相关方法的参数 %和_
 * 符号进行转义，便于模糊查询
 *
 * @author kuma
 * @version 2021.9
...
public class LbuWrapper<T> extends AbstractLambdaWrapper<T, LbuWrapper<T>>
    ...
    @Serial private static final long serialVersionUID = -4194344880194881367L;
    ...
    @Override
    public LbuWrapper<T> set(
            boolean condition, SFunction<T, ?> column, Object val, String mapping) {
        return null;
    }

    @Override
    public LbuWrapper<T> setSql(String setSql, Object... params) {
        return Update.super.setSql(setSql, params);
    }

    @Override
    public LbuWrapper<T> setSql(boolean condition, String sql, Object... params) {
    ...
    @Override
    public LbuWrapper<T> and(Consumer<LbuWrapper<T>> consumer) {
        return super.and(consumer);
    }

    @Override
    public LbuWrapper<T> or(Consumer<LbuWrapper<T>> consumer) {
    ...
    @Override
    public LbuWrapper<T> not(Consumer<LbuWrapper<T>> consumer) {
        return super.not(consumer);
    }

    @Override
    public <V> LbuWrapper<T> allEq(Map<SFunction<T, ?>, V> params) {
        return super.allEq(params);
    }

    @Override
    public <V> LbuWrapper<T> allEq(Map<SFunction<T, ?>, V> params, boolean null2IsNull) {
        return super.allEq(params, null2IsNull);
    }

    @Override
    public <V> LbuWrapper<T> allEq(
            BiPredicate<SFunction<T, ?>, V> filter, Map<SFunction<T, ?>, V> params) {
    ...
    @Override
    public LbuWrapper<T> notBetween(SFunction<T, ?> column, Object val1, Object val2) {
    ...
    public LbuWrapper<T> likeLeft(SFunction<T, ?> column, Object val) {
        return super.likeLeft(
                checkCondition(val), column, StrHelper.keywordConvert(val.toString()));
    }

    @Override
    public LbuWrapper<T> likeRight(SFunction<T, ?> column, Object val) {
        return super.likeRight(
                checkCondition(val), column, StrHelper.keywordConvert(val.toString()));
    }
    ...
    @Override
    public LbuWrapper<T> in(boolean condition, SFunction<T, ?> column, Object... values) {
    ...
    @Override
    public LbuWrapper<T> notIn(boolean condition, SFunction<T, ?> column, Object... values) {
    ...
    @Override
    public LbuWrapper<T> orderByAsc(SFunction<T, ?> column) {
        return super.orderByAsc(column);
    }

    @Override
    public LbuWrapper<T> orderByAsc(List<SFunction<T, ?>> columns) {
        return super.orderByAsc(columns);
    }

    @Override
    public LbuWrapper<T> orderByAsc(boolean condition, List<SFunction<T, ?>> columns) {
        return super.orderByAsc(condition, columns);
    }

    @Override
    public LbuWrapper<T> orderByDesc(SFunction<T, ?> column) {
    ...
    @Override
    public LbuWrapper<T> orderByDesc(boolean condition, List<SFunction<T, ?>> columns) {
    ...
    @Override
    public LbuWrapper<T> func(Consumer<LbuWrapper<T>> consumer) {
