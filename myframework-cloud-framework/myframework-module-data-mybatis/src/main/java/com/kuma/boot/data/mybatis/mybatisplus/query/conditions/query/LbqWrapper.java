...
package com.kuma.boot.data.mybatis.mybatisplus.query.conditions.query;
 ...
 *
 * <p>相比 LambdaQueryWrapper 的增强如下： 1，new QueryWrapper(T entity)时， 对entity 中的string字段 %和_
 * 符号进行转义，便于模糊查询 2，new QueryWrapper(T entity)时， 对entity 中 RemoteData 类型的字段 值为null或者
 * key为null或者""时，忽略拼接成查询条件 3，对nested、eq、ne、gt、ge、lt、le、in、*like*、 等方法 进行条件判断，null 或 "" 字段不加入查询
 * 4，对*like*相关方法的参数 %和_ 符号进行转义，便于模糊查询 5，增加 leFooter 方法， 将日期参数值，强制转换成当天 23：59：59 6，增加 geHeader 方法，
 * 将日期参数值，强制转换成当天 00：00：00
 *
 * @author kuma
 * @version 2021.9
...
public class LbqWrapper<T> extends AbstractLambdaWrapper<T, LbqWrapper<T>>
    ...
    @Serial private static final long serialVersionUID = -6842140106034506889L;
    ...
    public LbqWrapper<T> select(boolean condition, List<SFunction<T, ?>> columns) {
        if (condition && CollectionUtils.isNotEmpty(columns)) {
            this.sqlSelect.setStringValue(columnsToString(condition, columns));
        }
        return typedThis;
    }
    ...
    public LbqWrapper<T> like(SFunction<T, ?> column, Object val) {
        return super.like(
                this.checkCondition(val), column, StrHelper.keywordConvert(val.toString()));
    }
    ...
    public LbqWrapper<T> notLike(SFunction<T, ?> column, Object val) {
        return super.notLike(
                this.checkCondition(val), column, StrHelper.keywordConvert((String) val));
    }
