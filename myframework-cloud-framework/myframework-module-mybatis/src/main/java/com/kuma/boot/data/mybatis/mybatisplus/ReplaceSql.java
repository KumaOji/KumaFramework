...
public final class ReplaceSql {
    ...
    public static String replaceSql(DbType dbType, String schemaName, String sql) {
        SQLStatementParser parser =
                switch (dbType) {
                    case SQL_SERVER, SQL_SERVER2005 -> new SQLServerStatementParser(sql);
        ...
        if (sqlStatement instanceof SQLCallStatement sqlCallStatement) {
            ...
            if (expr instanceof SQLIdentifierExpr procedureName) {
                sqlCallStatement.setProcedureName(
                        new SQLPropertyExpr(schemaName, procedureName.getName()));
    ...
    private static void setSqlSchemaBySqlTableSource(
            String schemaName, SQLTableSource sqlTableSource) {
    ...
    private static void setSqlSchemaBySqlBinaryExpr(
            String schemaName, SQLBinaryOpExpr sqlBinaryOpExpr) {
    ...
    private static void setSqlSchemaBySelectQuery(
            String schemaName, SQLSelectQuery sqlSelectQuery) {
