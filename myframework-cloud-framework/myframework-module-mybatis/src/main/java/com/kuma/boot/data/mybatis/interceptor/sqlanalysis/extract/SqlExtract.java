...
import com.kuma.boot.data.mybatis.interceptor.sqlanalysis.util.MetaObjectUtil;
...
public class SqlExtract {
     ...
     * @return
     */
    public static com.kuma.boot.data.mybatis.interceptor.sqlanalysis.extract.SqlExtractResult extract(StatementHandler statementHandler) {
        ...
        if (delegateMetaObject.getValue("delegate") == null) {
            logger.warn(
                    "sql analysis get delegate null error,{}",
                    GsonUtil.bean2Json(statementHandler.getBoundSql()));
            return null;
        }
        if (!(delegateMetaObject.getValue("delegate")
                instanceof PreparedStatementHandler preparedStatementHandler)) {
            logger.info(
                    "sql analysis get delegate is not PreparedStatementHandler,{}",
                    GsonUtil.bean2Json(statementHandler.getBoundSql()));
            return null;
        }
        ...
        com.kuma.boot.data.mybatis.interceptor.sqlanalysis.extract.SqlExtractResult result = new com.kuma.boot.data.mybatis.interceptor.sqlanalysis.extract.SqlExtractResult();
    ...
    private static boolean needAnalysis(String sqlId, String sqlType) {
        ...
        // 检查间隔判断
        if (checkedIdMap.get(sqlId) != null
                && (System.currentTimeMillis() - checkedIdMap.get(sqlId))
                < SqlAnalysisConfig.getCheckInterval()) {
            return false;
        }
