...
public class BaseManager<M extends BaseMapper<T>, T> {
    ...
    protected Class<M> currentMapperClass() {
        return (Class<M>)
                ReflectionKit.getSuperClassGenericType(this.getClass(), BaseManager.class, 0);
    }

    @SuppressWarnings("unchecked")
    protected Class<T> currentModelClass() {
        return (Class<T>)
                ReflectionKit.getSuperClassGenericType(this.getClass(), BaseManager.class, 1);
    }
    ...
    protected <E> boolean executeBatch(
            Collection<E> list, int batchSize, BiConsumer<SqlSession, E> consumer) {
     ...
     * @return 对象列表
     */
    public List<T> findAllByFields(
            SFunction<T, ?> field, Collection<? extends Serializable> fieldValues) {
    ...
    public boolean existedByField(SFunction<T, ?> field, Object fieldValue, Serializable id) {
        String keyProperty = this.getKeyProperty();
        return query().eq(com.kuma.boot.data.mybatis.mybatisplus.MpUtils.getColumnName(field), fieldValue).ne(keyProperty, id).exists();
    }
