...
package com.kuma.boot.data.mybatis.interceptor.log.interceptor;
...
public class ActualSqlInterceptor implements Interceptor {
    private Logger log = LoggerFactory.getLogger(ActualSqlInterceptor.class);
     ...
     * 时间格式化
     */
    private static final FastDateFormat NORMAL_DATE_TIME_FORMAT =
                ...
                if (args.length == PARAMETER_COUNT_4) {
                    ...
                    cacheKey =
                            executor.createCacheKey(
                                    mappedStatement, parameter, rowBounds, boundSql);
            ...
            if (typeHandlerRegistry.hasTypeHandler(parameterObject.getClass())) {
                ...
                for (ParameterMapping parameterMapping : parameterMappings) {
                    ...
                    if (metaObject.hasGetter(property)) {
                        ...
                        String[] segments = StringUtils.split(property, '.').toArray(new String[0]);
                        MetaObject minMeta = metaObject;
                        String segment = null;
                        // 递归每一段 如果没有找到对应的getter则结束掉 将loopMeta置空 如果全都有 则循环结束后的minMeta为倒数第二段对象
                        // segment为最后一段的数据
     ...
     * @param result         执行结果
     */
    private void formatSqlLog(
            SqlCommandType sqlCommandType, String sqlId, String sql, long elapsed, Object result) {
        ...
        if (actualSqlProperties.isShowSql()) {
            ...
            Object endLine;
            if (!sql.isEmpty() && sql.charAt(sql.length() - 1) != SQL_DELIMITER) {
     ...
     */
    private Pair<String, Integer> replacePlaceholder(
            String sql, Object parameterObject, int start) {
        String result;
        if (parameterObject instanceof String
                || parameterObject instanceof JSONObject
                || parameterObject instanceof JSONArray) {
            // String类型要加单引号
            result =
                    SINGLE_QUOTE
                            + StringUtils.replace(
                            String.valueOf(parameterObject),
                            SINGLE_QUOTE,
