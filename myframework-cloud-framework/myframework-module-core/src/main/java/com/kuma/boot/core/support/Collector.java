...
import com.kuma.boot.common.model.Callable.Func0;
...
public class Collector {
    ...
    private Object createInstance(String key, Class<?> type) {
        try {
            ...
            if(obj instanceof Call callObj){
                //todo 需要改造
                callObj.set(() -> 0);
            }
        ...
        public Object run(String tag, Object obj, String methodName, Object[] params) {
            if (method == null) {
                Optional<Method> find = Arrays.stream(obj.getClass().getMethods())
                        .filter(c -> methodName.equalsIgnoreCase(c.getName()))
                        .findFirst();
                if (find.isEmpty()) {
    ...
    public static class Call {
        ...
        /** 设置回调 */
        public Call set(Func0<Object> func) {
            this.func = func;
            return this;
        }
    ...
    public static class SortList extends ConcurrentSkipListSet<SortInfo> {
        ...
        public void removeMore(int maxLength) {
            ...
            while (size() > maxLength) {
                ...
                if (count < -10) {
                    LogUtils.error("[严重bug] remove more,item:" + (last != null ? last.toString() : " 长时间无法移除导致死循环"));
                    break;
                }
            }
        }
