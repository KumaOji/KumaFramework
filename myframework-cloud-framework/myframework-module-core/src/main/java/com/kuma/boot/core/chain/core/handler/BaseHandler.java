package com.kuma.boot.core.chain.core.handler;


import com.kuma.boot.common.utils.log.LogUtils;
import com.kuma.boot.core.chain.core.context.HandlerContext;
...
public abstract class BaseHandler<P, R> {
    ...
    public void onCompleted(HandlerContext<P, R> context) {
        // 默认空实现
        LogUtils.debug("Handler completed: {}", this.getClass().getSimpleName());
    }
    ...
    public void onError(HandlerContext<P, R> context, Exception e) {
        // 默认空实现
        LogUtils.error("Handler error in {}: {}", this.getClass().getSimpleName(), e.getMessage(), e);
    }
}
