...
public @interface RStreamListener {
     ...
     * @return MessageModel
     */
    com.kuma.boot.cache.redis.stream.MessageModel messageModel() default com.kuma.boot.cache.redis.stream.MessageModel.CLUSTERING;

    /**
     * offsetModel，默认：LAST_CONSUMED
     *
     * <p>0-0 : 从开始的地方读。 $ ：表示从尾部开始消费，只接受新消息，当前 Stream 消息会全部忽略。 > :
     * 读取所有新到达的元素，这些元素的id大于消费组使用的最后一个元素。
     *
     * @return ReadOffsetModel
     */
    com.kuma.boot.cache.redis.stream.ReadOffsetModel offsetModel() default com.kuma.boot.cache.redis.stream.ReadOffsetModel.LAST_CONSUMED;
