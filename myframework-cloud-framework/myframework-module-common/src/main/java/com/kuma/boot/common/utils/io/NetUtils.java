...
public class NetUtils {
     ...
     *
     * <p>
     * This method is intended for use as a replacement of JDK method <code>
     * InetAddress.getLocalHost</code>, because that method is ambiguous on Linux systems.
     * Linux systems enumerate the loopback network interface the same way as regular LAN
     * network interfaces, but the JDK <code>InetAddress.getLocalHost</code> method does
     * not specify the algorithm used to select the address returned under such
     * circumstances, and will often return the loopback address, which is not valid for
    ...
    /** 断言处于联网状态 1. 这里可能会有一个问题，如果用户的 hosts 文件中有这个网址 */
    ...
    private static InetAddress findLocalAddress() {
        ...
        try {
            for (Enumeration<NetworkInterface> allInterfaces =
            ...
            for (String nifName : defaultNicList.split(PunctuationConstants.COMMA)) {
    ...
    private static InetAddress findAvailableAddress(NetworkInterface nic) {
        for (Enumeration<InetAddress> inetAddresses = nic.getInetAddresses();
