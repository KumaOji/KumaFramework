...
/**
 * 元组类型 用于表示多个不同类型的数据集合 可迭代 不可变，线程安全
 *
 * @author kuma
 * @version 2022.04
...
public abstract class Tuple implements Iterable<Object>, Serializable {
    ...
    public final Tuple subTuple(final int start, final int end) {
        if (start < 0 || end < 0) throw new IllegalArgumentException("start, end must >= 0");
        if (end >= this.valueList.size())
            throw new IllegalArgumentException("this tuple's size is" + this.valueList.size());
        int length = end - start + 1;
        if (length <= 0) throw new IllegalArgumentException("end must >= start");

        if (start == 0 && end == this.valueList.size() - 1) return this;
        ...
        switch (length) {
            ...
            case 3:
                return Tuple3.with(
                        this.valueList.get(start),
                        this.valueList.get(start + 1),
                        this.valueList.get(end));
            case 4:
                return Tuple4.with(
                        this.valueList.get(start),
                        this.valueList.get(start + 1),
                        this.valueList.get(start + 2),
                        this.valueList.get(end));
            case 5:
                return Tuple5.with(
                        this.valueList.get(start),
    ...
    public final Tuple add(final Tuple... tuples) {
        ...
        if (tuples.length == 0) return this;
        List<Object> temp = this.toList();
        for (Tuple t : tuples) temp.addAll(t.valueList);
        switch (temp.size()) {
            case 1:
    ...
    public final Tuple repeat(final int times) {
        if (times < 0) throw new IllegalArgumentException("times must >= 0");
    ...
    /**
     * 比较2个元组是否相同 元组里的所有位置上的元素都相同，即为元组相同
     * @param obj 需要比较的元组
     * @return 比较结果
     */
    @Override
    ...
    /**
     * 反转元组 反转后元组大小不变，子类各自实现可以达到最好性能，也可以指定返回值类型，方便使用
     * @return 反转后的元组
     */
