...
/**
 * 秒 分 时 日 月 周 (年) "0 15 10 ? * *" 每天上午10:15触发 "0 0/5 14 * * ?" 在每天下午2点到下午2:55期间的每5分钟触发 "0
 * 0-5 14 * * ?" 每天下午2点到下午2:05期间的每1分钟触发 "0 10,44 14 ? 3 WED" 三月的星期三的下午2:10和2:44触发 "0 15 10
 * ? * MON-FRI" 周一至周五的上午10:15触发
 * -------------------------------------------------------------------
 * 一、根据cron表达式，计算某天的那些时刻执行。 思路：1、切割cron表达式 2、转换每个域 3、计算执行时间点（关键算法，解析cron表达式）
 * 4、计算某一天的哪些时间点执行 二、根据cron表达式，给定Date，计算下一个执行时间点 思路：1、找到所有时分秒的组合并按照时分秒排序
 * 2、给定的时分秒在以上集合之前、之后处理 3、给定时时分秒在以上集合中找到一个最小的位置 4、day+1循环直到找到满足月、星期的那一天
...
public class CronUtil {
    ...
    public static Date doNext(
            Calendar calendar,
            CronField fieldSecond,
            CronField fieldMinute,
            CronField fieldHour,
            CronField fieldDay,
            CronField fieldMonth,
            CronField fieldWeek,
            CronField fieldYear) {
        ////////////////////////////////// 时分秒///////////////////////////////
    ...
    /** 处理日月周 */
    private static Date doDayOfYear(
            int addYear,
            Calendar calendar,
            CronField fieldDay,
            CronField fieldMonth,
            CronField fieldWeek,
            CronField fieldYear,
            TimeOfDay timeOfDayMin) {
        if (addYear >= MAX_ADD_YEAR) {
            throw new IllegalArgumentException(
                    "Invalid cron expression【日月周年】 which led to runaway search for next trigger");
        }
        ...
        if (null == dayOfYearMin) {
            addOneYear(calendar, timeOfDayMin);
            return doDayOfYear(
                    ++addYear, calendar, fieldDay, fieldMonth, fieldWeek, fieldYear, timeOfDayMin);
        }
    ...
    /** 找到最小的满足日月周的 */
    private static DayOfYear findMinDayOfYear(
    ...
    /** 处理时分秒，并返回最小的时分秒 */
    private static TimeOfDay doTimeOfDay(
            Calendar calendar, CronField fieldSecond, CronField fieldMinute, CronField fieldHour) {
        // 先确定时分秒
    ...
    public static List<TimeOfDay> timesOfDay(
            CronField fieldHour, CronField fieldMinute, CronField fieldSecond) {
        ...
        List<TimeOfDay> points =
                new ArrayList<>(listHour.size() * listMinute.size() * listSecond.size());
    ...
    public static List<CronField> convertCronField(String cron) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            cronFields.add(
                    new CronField(cronPosition, CronShapingUtil.shaping(cut.get(i), cronPosition)));
        }
        return cronFields;
    }
