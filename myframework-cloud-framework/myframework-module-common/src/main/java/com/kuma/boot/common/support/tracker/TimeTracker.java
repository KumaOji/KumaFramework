 ...
 * <h2>使用示例：</h2>
 *
 * <h3>try-with-resources 手动跟踪</h3> <pre>{@code
 * // 手动管理资源和性能跟踪
 ...
 * }</pre>
 *
 * <h3>结合静态方法的try-with-resources</h3> <pre>{@code
 ...
 *
 * <p>
 * 注意：使用try-with-resources可以确保资源正确关闭， 并自动记录执行时间。
 * </p>
 *
 * <h3>lambda自动处理异常</h3> <pre>{@code
 * // 无返回值方法
 ...
 *
 * <p>
 * 注意：默认情况下会打印执行时间到控制台。对于生产环境， 建议根据需要自定义日志记录机制。
 * </p>
 *
 * @author kuma
 */
public class TimeTracker implements AutoCloseable {
     ...
     */
    public static <T> T trackThrows(String operationName, ThrowableSupplier<T> execution)
            throws Exception {
     ...
     * @throws Exception 如果执行过程中发生异常
     */
    public static void trackThrows(String operationName, ThrowableRunnable execution)
            throws Exception {
        try (TimeTracker ignored = new TimeTracker(operationName, true)) {
            execution.run();
        }
