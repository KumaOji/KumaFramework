...
public final class Tuples {
    ...
    public static <T> void sort(
            final List<? extends Tuple> list, final int index, final Comparator<T> comparator) {
        requireNonNull(list, "list is null");
        if (list.size() < 2) return;
        requireNonNull(comparator, "comparator is null");
        if (index < 0) throw new IllegalArgumentException("index must >= 0");
     ...
     */
    public static <T> void sort(
            final Tuple[] array, final int index, final Comparator<T> comparator) {
