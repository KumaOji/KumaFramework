...
public final class OrikaUtils {
    ...
    /**
     * 映射实体（自定义配置） Map<String,String> refMap = new HashMap<>(1); //map key 放置 源属性，value 放置
     * 目标属性 refMap.put("email","emailAddress"); Teacher teacher =
     * OrikaUtils.convert(student, Teacher.class, refMap);
     ...
     * @return 映射类对象
     */
    public static <S, D> List<D> converts(
            Collection<S> source, Class<D> targetClass, Map<String, String> configMap) {
        ...
        MapperFacade mapperFacade =
                getMapperFacade(targetClass, source.iterator().next().getClass(), configMap);
     ...
     * @return 映射类对象
     */
    private static <S, D> MapperFacade getMapperFacade(
            Class<S> sourceClass, Class<D> targetClass, Map<String, String> configMap) {
     ...
     * @return 目标对象对象数组
     */
    public static <S, D> D[] convertArray(
            final S[] source, final D[] target, final Class<D> targetClass) {
    ...
    public static class LocalDateTimeToString
        ...
        public String convertTo(
                LocalDateTime dateTime, Type<String> type, MappingContext mappingContext) {
            return dateTime.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        }

        @Override
        public LocalDateTime convertFrom(
                String s, Type<LocalDateTime> type, MappingContext mappingContext) {
    ...
    public static class TimestampToString extends BidirectionalConverter<Timestamp, String> {
        ...
        @Override
        public Timestamp convertFrom(
                String s, Type<Timestamp> type, MappingContext mappingContext) {
            return null;
        }
    }
    ...
    public static <S, D> List<D> mapList(
        ...
        return MAPPER_FACADE.mapAsList(
                sourceList,
