...
public class ByteCodeLoader extends SecureClassLoader {
    ...
    private static final ConcurrentMap<String, Class<?>> JAVA_FILE_OBJECT_MAP =
    ...
    /**
     * Creates a new {@code ByteCodeLoader} ready to load a class with the given name and
     * the given byte code.
    ...
    /**
     * Utility method for creating a new {@code ByteCodeLoader} and then directly load the
     * given byte code.
     * @param className The name of the class
    ...
    public static Class<?> load(String className, byte[] byteCode) {
        CheckedFunction<String, Class<?>> classLoadFunc =
                (key) -> new ByteCodeLoader(key, byteCode).loadClass(className);
        return CollectionUtils.computeIfAbsent(
                JAVA_FILE_OBJECT_MAP, className, Unchecked.function(classLoadFunc));
    }
