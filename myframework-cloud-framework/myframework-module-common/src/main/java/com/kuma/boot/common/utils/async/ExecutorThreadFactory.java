 ...
 *
 * <p>
 * The thread factory can be given an {@link UncaughtExceptionHandler} for the threads. If
 * no handler is explicitly given, the default handler for uncaught exceptions will log
 * the exceptions and kill the process afterwards. That guarantees that critical
 * exceptions are not accidentally lost and leave the system running in an inconsistent
 * state.
 *
 * <p>
 * Threads created by this factory are all called '(pool-name)-thread-n', where
 * <i>(pool-name)</i> is configurable, and <i>n</i> is an incrementing number.
 *
 * <p>
 * All threads created by this factory are daemon threads and have the default (normal)
 * priority.
 */
public class ExecutorThreadFactory implements ThreadFactory {
     ...
     * ('flink-executor-pool') and the default uncaught exception handler (log exception
     * and kill process).
     */
