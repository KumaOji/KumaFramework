...
public final class WeakTypeUtils {
    ...
    /**
     * 将对象转换为boolean 仿照JS的规则 null -> false false -> false true -> true 0 -> false !0 ->
     * true "" -> false !"" -> true object -> true array -> true
     * @param obj 对象
     * @return 布尔
     */
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> T and(
            Supplier<T> supplier1, Supplier<T> supplier2, Supplier<T>... suppliers) {
        Stream<Supplier<T>> stream = Stream.of(supplier1, supplier2);
        if (suppliers != null && suppliers.length > 0) {
            stream = Stream.concat(stream, Arrays.stream(suppliers));
        }
     ...
     * @param <T> 提供对象的类型
     */
    public static <T> void ifThen(
            Supplier<T> supplier, Predicate<T> predicate, Consumer<T> consumer) {
        T t = supplier.get();
        if (predicate.test(t)) {
            consumer.accept(t);
        }
     ...
     */
    public static <T> void ifElse(
            T t, Predicate<T> predicate, Runnable tRunnable, Runnable fRunnable) {
