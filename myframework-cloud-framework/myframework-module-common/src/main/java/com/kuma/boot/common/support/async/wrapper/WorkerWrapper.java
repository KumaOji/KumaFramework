...
public class WorkerWrapper<T, V> {
     ...
     * <p>
     * 1 4 -------3 2 如这种在4执行前，可能3已经执行完毕了（被2执行完后触发的），那么4就没必要执行了。 注意，该属性仅在nextWrapper数量<=1时有效，>1时的情况是不存在的
     */
    ...
    private void work(
        ...
        /*
         * 如果有前方依赖，存在两种情况 一种是前面只有一个wrapper。即 A -> B 一种是前面有多个wrapper。A C D ->
    ...
    private void beginNext( ExecutorService executorService, long now, long remainTime ) {
        ...
        CompletableFuture<Void>[] futures = new CompletableFuture[nextWrappers.size()];
    ...
    private synchronized void doDependsJobs(
            ExecutorService executorService,
