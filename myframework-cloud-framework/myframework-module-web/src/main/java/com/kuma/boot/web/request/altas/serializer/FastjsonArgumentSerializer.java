...
/**
 * 基于Fastjson的JSON参数序列化器 使用阿里巴巴 Fastjson2 进行序列化，解决循环引用等序列化问题
 *
 * @author nemoob
 * @since 0.2.0
 */

public class FastjsonArgumentSerializer implements com.kuma.boot.web.request.altas.serializer.ArgumentSerializer {

    private final SensitiveDataMasker sensitiveDataMasker;
    private final com.kuma.boot.web.request.altas.serializer.ArgumentFormatConfig argumentFormatConfig;

    public FastjsonArgumentSerializer(
            SensitiveDataMasker sensitiveDataMasker,
            com.kuma.boot.web.request.altas.serializer.ArgumentFormatConfig argumentFormatConfig) {
        this.sensitiveDataMasker = sensitiveDataMasker;
        this.argumentFormatConfig = argumentFormatConfig != null ? argumentFormatConfig : new com.kuma.boot.web.request.altas.serializer.ArgumentFormatConfig();
    }

    // 兼容旧的构造函数
    public FastjsonArgumentSerializer( SensitiveDataMasker sensitiveDataMasker) {
        this.sensitiveDataMasker = sensitiveDataMasker;
        this.argumentFormatConfig = new com.kuma.boot.web.request.altas.serializer.ArgumentFormatConfig();
    }
    ...
    public String serializeArgs(Object[] args, Log annotation) {
        ...
        try {
            ...
            // 根据配置选择序列化格式
            String result;
            if (argumentFormatConfig.getType() == com.kuma.boot.web.request.altas.serializer.ArgumentFormatType.KEY_VALUE) {
            ...
            LogUtils.warn("Fastjson parameter serialization failed", e);
    ...
    public String serializeResult(Object result, Log annotation) {
        ...
        try {
            ...
            LogUtils.warn("Fastjson result serialization failed", e);
    ...
    public String serialize(Object obj, int maxLength) {
        ...
        try {
            ...
            LogUtils.warn("Fastjson object serialization failed: {}", obj.getClass().getSimpleName(), e);
        ...
        try {
            ...
            for (Object arg : args) {
                if (arg == null) {
                    ...
                    simplifiedArgs.add(
                            "[" + arg.getClass().getSimpleName() + "@" + Integer.toHexString(arg.hashCode()) + "]");
                }
            }
            return JSON.toJSONString(simplifiedArgs);
        } catch (Exception e) {
            LogUtils.debug("Fallback serialization also failed", e);
    ...
    private String serializeResultWithFallback(Object result, Log annotation, Exception originalException) {
        try {
            ...
            LogUtils.debug("Fallback result serialization also failed", e);
            return "[SERIALIZATION_FAILED: " + originalException.getMessage() + "]";
        }
    }
    ...
    private String serializeObjectWithFallback(Object obj, int maxLength, Exception originalException) {
        try {
            ...
            LogUtils.debug("Fallback object serialization also failed", e);
            return "[SERIALIZATION_FAILED: " + originalException.getMessage() + "]";
        }
    }
