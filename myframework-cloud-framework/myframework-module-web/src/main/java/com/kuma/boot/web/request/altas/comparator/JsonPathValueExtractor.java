...
/**
 * JsonPath 值提取器 用于从对象中提取指定路径的值，并进行比较
 * <p>
 * 核心功能： 1. 使用 JsonPath 从对象中提取指定路径的值 2. 支持多个路径的批量提取 3. 提供值比较功能 4. 处理提取失败的情况
 *
 * @author nemoob
 * @since 0.2.0
 */
public class JsonPathValueExtractor {
    ...
    public Object extractValue(Object obj, String jsonPath) {
        ...
        try {
            ...
            LogUtils.debug("Path not found: {} in object: {}", jsonPath, obj.getClass().getSimpleName());
            return null;
        }
        catch (Exception e) {
            LogUtils.debug("Failed to extract value from path: {} in object: {}", jsonPath,
            ...
            for (String jsonPath : jsonPaths) {
                if (jsonPath != null && !jsonPath.trim().isEmpty()) {
                    try {
                        ...
                        LogUtils.debug("Path not found: {}", jsonPath);
                        result.put(jsonPath, null);
                    } catch (Exception e) {
                        LogUtils.debug("Failed to extract value from path: {}", jsonPath, e);
                        result.put(jsonPath, null);
                    }
                }
            }
        } catch (Exception e) {
            LogUtils.debug("Failed to serialize object for value extraction: {}", obj.getClass().getSimpleName(), e);
        }
    ...
    public boolean hasPath(Object obj, String jsonPath) {
        ...
        try {
            ...
            LogUtils.debug("Error checking path existence: {}", jsonPath, e);
            return false;
        }
    }
    ...
    public List<String> getAvailablePaths(Object obj) {
        ...
        try {
            ...
            LogUtils.debug("Failed to discover paths in object: {}", obj.getClass().getSimpleName(), e);
        }
    ...
    private List<String> discoverPaths(DocumentContext context, String basePath) {
        ...
        try {
            ...
            LogUtils.debug("Failed to discover paths from base: {}", basePath, e);
        }
