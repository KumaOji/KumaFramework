...
import com.kuma.boot.web.request.altas.annotation.*;
...
/**
 * 日志切面处理器 拦截带有@Log注解的方法，实现自动日志记录
 *
 * @author nemoob
 * @since 0.2.0
 */
...
public class AtlasLogAspect {
     ...
     * 环绕通知：拦截带有@Log注解的方法
     */
    @Around("@annotation(com.kuma.boot.web.request.altas.annotation.Log)"
            + " || @annotation(com.kuma.boot.web.request.altas.annotation.Logs) "
            + "|| @annotation(com.kuma.boot.web.request.altas.annotation.JsonPathCompare)")
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (method.isAnnotationPresent(com.kuma.boot.web.request.altas.annotation.JsonPathCompare.class)) {
            beforeArgs = deepCopyArgs(args);
        }

        // 获取所有Log注解
        List<com.kuma.boot.web.request.altas.annotation.Log> logAnnotations = getAllLogAnnotations(method);
        if (logAnnotations.isEmpty() && !method.isAnnotationPresent(com.kuma.boot.web.request.altas.annotation.JsonPathCompare.class)) {
            return joinPoint.proceed();
        }

        // 检查方法是否被@LogIgnore标记
        if (method.isAnnotationPresent(com.kuma.boot.web.request.altas.annotation.LogIgnore.class)) {
            return joinPoint.proceed();
        }
        ...
        if (traceId == null) {
            LogUtils.warn("TraceId is null in AOP, Filter may not be working properly, method: {}", method.getName());
        ...
        try {
            // 记录进入日志
            for (com.kuma.boot.web.request.altas.annotation.Log logAnnotation : logAnnotations) {
            ...
            // 处理 JsonPath 比较
            if (jsonPathCompareProcessor != null && method.isAnnotationPresent(com.kuma.boot.web.request.altas.annotation.JsonPathCompare.class)) {
     ...
     * 记录方法进入日志
     */
    private void recordEnterLog(com.kuma.boot.web.request.altas.annotation.Log logAnnotation, Method method, Object[] args, String traceId) {
        ...
        try {
            ...
            LogUtils.warn("Failed to record enter log: {}", method.getName(), e);
        }
    }
     ...
     * 记录方法退出日志
     */
    private void recordExitLog(com.kuma.boot.web.request.altas.annotation.Log logAnnotation, Method method, Object[] args,
        ...
        try {
            ...
            LogUtils.warn("Failed to record exit log: {}", method.getName(), e);
        }
    }
     ...
     * 记录异常日志
     */
    private void recordExceptionLog(com.kuma.boot.web.request.altas.annotation.Log logAnnotation, Method method, Object[] args,
        ...
        try {
            // 查找匹配的异常处理器
            com.kuma.boot.web.request.altas.annotation.ExceptionHandler exceptionHandler = findExceptionHandler(exception, logAnnotation.exceptionHandlers());

            String message;
            com.kuma.boot.web.request.altas.annotation.LogLevel logLevel;
            ...
            if (exceptionHandler != null) {
                ...
                logLevel = com.kuma.boot.web.request.altas.annotation.LogLevel.ERROR;
                logStackTrace = true;
            }
            ...
            LogUtils.warn("Failed to record exception log: {}", method.getName(), e);
        }
    }
     ...
     * 判断是否应该记录日志
     */
    private boolean shouldLog(com.kuma.boot.web.request.altas.annotation.Log logAnnotation, Method method, Object[] args, Object result, Throwable exception) {
        // 检查条件表达式
        if (!logAnnotation.condition().isEmpty()) {
            ...
            try {
                ...
                LogUtils.warn("Condition expression evaluation failed, logging by default: {}",
                        logAnnotation.condition(), e);
                return true;
            }
        }
     ...
     * 构建日志消息
     */
    private String buildLogMessage(com.kuma.boot.web.request.altas.annotation.Log logAnnotation, LogContext logContext, boolean isException) {
        // 在调用SpEL求值前验证LogContext状态
        if (logContext == null) {
            LogUtils.warn("buildLogMessage received null LogContext");
            return "[LogContext为null]";
        }

        if (logContext.getArgs() == null) {
            LogUtils.warn("LogContext.args is null in buildLogMessage, method: {}", logContext.getMethodName());
        }
     ...
     * 构建日志详细信息
     */
    private String buildLogDetails(com.kuma.boot.web.request.altas.annotation.Log logAnnotation, LogContext logContext, Method method) {
     ...
     * 序列化参数
     */
    private String serializeArgs(Object[] args, com.kuma.boot.web.request.altas.annotation.Log logAnnotation, Parameter[] parameters, Method method) {
        // 检查注解是否指定了自定义格式化器
        ...
        if (formatterName != null && !formatterName.trim().isEmpty()) {
            ...
            LogUtils.debug("Using custom formatter '{}' for method {}.{}", formatterName, className, methodName);
     ...
     * 序列化返回值
     */
    private String serializeResult(Object result, com.kuma.boot.web.request.altas.annotation.Log logAnnotation, Method method) {
        // 检查注解是否指定了自定义格式化器
    ...
    private LogContext createLogContext(Method method, Object[] args, Object result,
        ...
        context.setArgs(safeArgs);  // 使用安全副本
        context.setResult(result);
        context.setException(exception);
        context.setExecutionTime(executionTime);

        // 验证创建的LogContext状态
        LogUtils.debug("创建LogContext: method={}, args={}, argsLength={}",
                method.getName(),
     ...
     * 获取所有Log注解
     */
    private List<com.kuma.boot.web.request.altas.annotation.Log> getAllLogAnnotations(Method method) {
        List<com.kuma.boot.web.request.altas.annotation.Log> annotations = new ArrayList<>();

        // 检查方法级别的注解
        com.kuma.boot.web.request.altas.annotation.Log singleLog = AnnotationUtils.findAnnotation(method, com.kuma.boot.web.request.altas.annotation.Log.class);
        if (singleLog != null) {
            annotations.add(singleLog);
        }

        com.kuma.boot.web.request.altas.annotation.Logs multiLogs = AnnotationUtils.findAnnotation(method, com.kuma.boot.web.request.altas.annotation.Logs.class);
        if (multiLogs != null) {
        ...
        com.kuma.boot.web.request.altas.annotation.Log classSingleLog = AnnotationUtils.findAnnotation(clazz, com.kuma.boot.web.request.altas.annotation.Log.class);
        if (classSingleLog != null) {
            annotations.add(classSingleLog);
        }

        com.kuma.boot.web.request.altas.annotation.Logs classMultiLogs = AnnotationUtils.findAnnotation(clazz, com.kuma.boot.web.request.altas.annotation.Logs.class);
     ...
     * 查找匹配的异常处理器
     */
    private com.kuma.boot.web.request.altas.annotation.ExceptionHandler findExceptionHandler(Throwable exception, com.kuma.boot.web.request.altas.annotation.ExceptionHandler[] handlers) {
        ...
        // 精确匹配
        for (com.kuma.boot.web.request.altas.annotation.ExceptionHandler handler : handlers) {
     ...
     * 根据级别记录日志
     */
    private void logWithLevel(Logger logger, com.kuma.boot.web.request.altas.annotation.LogLevel level,
