...
public class LoggingFilter implements Filter {
    ...
    public LoggingFilter( LogConfigProperties properties, ArgumentFormatConfig argumentFormatConfig) {
        ...
        // ✅ 添加调试日志
        LogUtils.debug("=== LoggingFilter Constructor Debug ===");
        LogUtils.debug("Received LogConfigProperties: {}", properties);
        if (properties != null && properties.getHttpLog() != null) {
            LogUtils.debug("HTTP Log urlFormat: '{}'", properties.getHttpLog().getUrlFormat());
            LogUtils.debug("HTTP Log includeQueryString: {}", properties.getHttpLog().isIncludeQueryString());
        } else {
            LogUtils.debug("No HTTP log configuration found in properties");
        }
        LogUtils.debug("===========================================");
    }
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        LogUtils.info("Atlas Log filter initialized successfully");
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (traceId == null) {
            ...
            if (traceId == null || traceId.trim().isEmpty()) {
                ...
                LogUtils.debug("LoggingFilter generated new TraceId: {}", traceId);
            } else {
                LogUtils.debug("LoggingFilter obtained TraceId from request header: {}", traceId);
            }
            // 设置到当前线程
        ...
        LogUtils.debug("LoggingFilter started - TraceId: {}", traceId);
        ...
        try {
            ...
            LogUtils.debug("LoggingFilter finished - saved TraceId: {}, current TraceId: {}", traceId, currentTraceId);
    ...
    private void logRequestStart(HttpServletRequest request, String traceId) {
        if (LogUtils.isDebugEnabled()) {
            ...
            LogUtils.debug(logMsg.toString());
        }
    }
    ...
    private void logRequestEnd(HttpServletRequest request, HttpServletResponse response,
        ...
        // 根据执行时间和状态码选择日志级别
        if (status >= 500) {
            LogUtils.error(logMsg.toString());
        } else if (status >= 400) {
            LogUtils.warn(logMsg.toString());
        } else if (executionTime > properties.getPerformance().getSlowThreshold()) {
            LogUtils.warn(logMsg.toString() + " | SlowRequest: true");
        } else if (LogUtils.isInfoEnabled()) {
            LogUtils.info(logMsg.toString());
        }
    }
    ...
    @Override
    public void destroy() {
        LogUtils.info("Atlas Log filter destroyed");
    }
}
