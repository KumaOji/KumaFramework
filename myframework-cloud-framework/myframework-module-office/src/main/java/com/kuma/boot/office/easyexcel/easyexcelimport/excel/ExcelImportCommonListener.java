...
public class ExcelImportCommonListener<T> implements ReadListener<T> {
    ...
    /**
     * 在转换异常 获取其他异常情况下会调用本接口。抛出异常则停止读取。如果这里不抛出异常则 继续读取下一行。 根据自己需要看是否抛出异常，如果没有特殊要求，则可以不修改。
     *
     * @param exception
     * @param context
     * @throws Exception
    ...
    private void persistentDataToDb(List<T> data) {
        // 对数据分组，批量插入
//        List<List<T>> dataList = avgPartition(data, ImportConstant.MAX_INSERT_COUNT);
