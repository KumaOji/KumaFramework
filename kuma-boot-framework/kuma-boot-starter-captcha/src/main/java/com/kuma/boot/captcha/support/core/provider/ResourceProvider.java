...
public class ResourceProvider implements InitializingBean {
    ...
    private void loadFonts() {
        if (MapUtils.isEmpty(this.fonts)) {
            this.fonts = getFonts("classpath*:/fonts/*.ttf");
            log.debug("[kmc] |- Font load complete, total number is [{}]", this.fonts.size());
        }

    }

    private Font getDefaultFont(String fontName, int fontSize, FontStyle fontStyle) {
        return StringUtils.isNotBlank(fontName) ? new Font(fontName, fontStyle.getMapping(), fontSize) : new Font("WenQuanYi Zen Hei", fontStyle.getMapping(), fontSize);
    }

    public Font getFont(String fontName, int fontSize, FontStyle fontStyle) {
        return !MapUtils.isEmpty(this.fonts) && !ObjectUtils.isEmpty(this.fonts.get(fontName)) ? ((Font)this.fonts.get(fontName)).deriveFont(fontStyle.getMapping(), Integer.valueOf(fontSize).floatValue()) : this.getDefaultFont(fontName, fontSize, fontStyle);
    }
    ...
    private String getRandomBase64Image(Map<String, String> container, CaptchaResource captchaResource) {
        String[] data = (String[])this.imageIndexes.get(captchaResource.name());
        if (ArrayUtils.isNotEmpty(data)) {
            int randomInt = RandomProvider.randomInt(0, data.length);
            return (String)container.get(data[randomInt]);
        } else {
            return null;
        }
    }
