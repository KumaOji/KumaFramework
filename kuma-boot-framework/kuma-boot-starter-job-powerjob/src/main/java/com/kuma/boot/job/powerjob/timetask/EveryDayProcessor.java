...
/**
 * 每日定时任务处理器
 * <p>在 PowerJob 控制台配置 CRON 表达式 {@code 0 0 0 * * ?}，处理器为本类全限定名</p>
 *
 * @author kuma
 * @version 2024.1
 * @since 2024-01-01
...
public class EveryDayProcessor implements BasicProcessor {
    ...
    public EveryDayProcessor(List<EveryDayExecute> everyDayExecutes) {
    ...
    public ProcessResult process(TaskContext context) throws Exception {
        ...
        for (EveryDayExecute task : everyDayExecutes) {
            try {
                ...
                LogUtils.error("每日任务异常: {}", e.getMessage(), e);
                context.getOmsLogger().error("每日任务异常: {} - {}", task.getClass().getSimpleName(), e.getMessage());
