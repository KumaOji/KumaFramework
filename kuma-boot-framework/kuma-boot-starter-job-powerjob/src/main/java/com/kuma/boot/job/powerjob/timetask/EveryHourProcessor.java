...
/**
 * 每小时定时任务处理器
 * <p>在 PowerJob 控制台配置 CRON 表达式 {@code 0 0 * * * ?}，处理器为本类全限定名</p>
 *
 * @author kuma
 * @version 2024.1
 * @since 2024-01-01
 */
public class EveryHourProcessor implements BasicProcessor {

    private final List<EveryHourExecute> everyHourExecutes;

    public EveryHourProcessor(List<EveryHourExecute> everyHourExecutes) {
    ...
    public ProcessResult process(TaskContext context) throws Exception {
        ...
        for (EveryHourExecute task : everyHourExecutes) {
            try {
                ...
                LogUtils.error("每小时任务异常: {}", e.getMessage(), e);
                context.getOmsLogger().error("每小时任务异常: {} - {}", task.getClass().getSimpleName(), e.getMessage());
