...
public interface BaseSuperService<T extends SuperEntity<T, I>, I extends Serializable> extends IService<T> {
    ...
    * 获取mapper类型
    *
    * @return {@link MpSuperMapper }<{@link T }, {@link I }>
    ...
    * 获取repository类型
    *
    * @return {@link JpaSuperRepository }<{@link T }, {@link I }>
    * @since 2022-09-13 09:30:42
    */
    ...
    * @since 2022-09-13 09:30:42
    */
   boolean saveOrUpdateIdempotency(
           T entity, DistributedLock lock, String lockKey, Predicate predicate, Wrapper<T> countWrapper, String msg);
   ...
   default long jpaCount(Example<T> example) {
      return respository().count(example);
   }
   ...
   default List<T> jpaFindAll(Example<T> example, Sort sort) {
      return respository().findAll(example, sort);
   }
   ...
   default Page<T> jpaFindByPage(Example<T> example, Pageable pageable) {
      return respository().findAll(example, pageable);
   }
   ...
   default Page<T> jpaFindByPage(Example<T> example, int pageNumber, int pageSize) {
      return respository().findAll(example, PageRequest.of(pageNumber, pageSize));
   }
   ...
   default void jpaDeleteAll(Iterable<T> entities) {
      respository().deleteAll(entities);
   }
   ...
   default T jpaSave(T domain) {
      return respository().save(domain);
   }
   ...
   default <S extends T> List<S> jpaSaveAll(Iterable<S> entities) {
      return respository().saveAll(entities);
   }
