...
import static com.baomidou.mybatisplus.core.toolkit.ClassUtils.newInstance;
...
public interface BaseBatchController<T extends SuperEntity<T, I>, I extends Serializable, SaveDTO, UpdateDTO>
    ...
    */
   //	@Operation(summary = "通用批量操作",
   //		description = "通用批量操作",
   //		requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
   //			description = "通用批量操作DTO",
   //			required = true,
   ...
   default Boolean batchCreate(List<SaveDTO> saveDTOList) {
      ...
      List<T> entityList = saveDTOList.stream()
              ...
              .map(saveDTO -> {
                 T t = newInstance(getEntityClass());
   ...
   default Boolean batchUpdate(List<BatchUpdate<UpdateDTO, I>> updateDTOList) {
      ...
      List<I> ids = updateDTOList.stream().map(BatchUpdate::id).toList();
