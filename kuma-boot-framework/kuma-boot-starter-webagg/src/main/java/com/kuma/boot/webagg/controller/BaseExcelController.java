...
public interface BaseExcelController<T extends SuperEntity<T, I>, I extends Serializable, QueryDTO, QueryVO>
   ...
   default void export(
           @Validated @RequestBody QueryDTO params, HttpServletRequest request, HttpServletResponse response) {
   ...
   @Parameters({@Parameter(name = "file", required = true, description = "文件信息", schema = @Schema(type = "file"))})
   ...
   default Result<Boolean> importExcel(
      ...
      params.setTitleRows(
              StrUtil.isEmpty(request.getParameter("titleRows"))
                      ? 0
                      : Convert.toInt(request.getParameter("titleRows")));
      params.setHeadRows(
              StrUtil.isEmpty(request.getParameter("headRows"))
                      ? 1
