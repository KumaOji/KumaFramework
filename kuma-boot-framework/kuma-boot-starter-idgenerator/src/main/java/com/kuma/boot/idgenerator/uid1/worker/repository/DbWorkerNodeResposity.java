...
public class DbWorkerNodeResposity implements WorkerNodeResposity {
   ...
   public WorkerNodeEntity getWorkerNodeByHostPort(String host, String port) {
      return (WorkerNodeEntity)this.jdbcTemplate.queryForObject("SELECT ID,HOST_NAME,PORT,TYPE,LAUNCH_DATE,MODIFIED,CREATED FROM WORKER_NODE WHERE HOST_NAME = ? AND PORT = ?", (rs, rowNum) -> {
   ...
   public void addWorkerNode(WorkerNodeEntity entity) {
      ...
      PreparedStatementCreator preparedStatementCreator = (con) -> {
         PreparedStatement ps = con.prepareStatement("INSERT INTO ``WORKER_NODE`` (HOST_NAME,PORT,TYPE,LAUNCH_DATE,MODIFIED,CREATED) VALUES (?,?,?,?,NOW(),NOW())", 1);
      ...
      entity.setId(((Number)Objects.requireNonNull(keyHolder.getKey())).longValue());
   }
}
