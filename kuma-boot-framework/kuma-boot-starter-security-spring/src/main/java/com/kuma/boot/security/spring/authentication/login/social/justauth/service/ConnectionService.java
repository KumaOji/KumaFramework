 ...
 * {@link AuthUser}. Allows for implicitly creating a local user profile from connection data during
 * a provider sign-in attempt. note: To replace the implementation logic of the built-in
 * {@code auth_token} and {@code user_connection} tables, implement this interface to inject the IOC
 * container, And set the property ums.repository.enableStartUpInitializeTable = false.
 *
 * @author YongWu zheng
...
public interface ConnectionService {
    ...
    /**
     * Sign up a new user of the application from the connection. 如果 {@code authUser.getUsername()}
     * 重名, 则使用 {@code authUser.getUsername() + "_" + authUser.getSource()} 或
    ...
    /**
     * 第三方授权登录信息{@link AuthUser}绑定到本地账号{@link UserDetails}, 且添加第三方授权登录信息到 user_connection 与
     * auth_token
     *
     ...
     */
    void binding(
            @NonNull UserDetails principal, @NonNull AuthUser authUser, @NonNull String providerId);
    ...
    /**
     * 获取当前账号下所有绑定的第三方账号接口.<br> Find all connections the current user has across all providers. The
