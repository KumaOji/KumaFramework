...
/**
 * 第三方授权登录用户信息增删改查, 绑定与解绑及查询是否绑定与解绑接口. A data access interface for managing a global store of users
 * connections to service providers. Provides data access operations.
 *
 * @author Keith Donald
 * @author YongWu zheng
...
public interface UsersConnectionRepository {
    ...
    /**
     * 获取 userId 的所有绑定信息. Find all connections the current user has across all providers. The
     * returned map contains an entry for each provider the user is connected to. The key for each
     * entry is the providerId, and the value is the list of {@link ConnectionData}s that exist
     * between the user and that provider. For example, if the user is connected once to Facebook
     * and twice to Twitter, the returned map would contain two entries with the following
     * structure:
     * <pre>
     * {
     *     "qq" -&gt; Connection("Jack") ,
     ...
     * }
     * </pre>
     * The returned map is sorted by providerId and entry values are ordered by rank. Returns an
     * empty map if the user has no connections.
     *
    ...
    /**
     * 获取 userId 和 providerId 的所以绑定信息. Find the connections the current user has to the provider
     * registered by the given id e.g. 'qq'. The returned list is ordered by connection rank.
    ...
    /**
     * 获取 userId 和 providerUserIds 的所以绑定信息. Find the connections the current user has to the given
     * provider users. The providerUsers parameter accepts a map containing an entry for each
     * provider the caller is interested in. The key for each entry is the providerId e.g. "qq", and
     * the value is a list of provider user ids to fetch connections to e.g. ("987665", "435796",
     * "584444"). The returned map has the same structure and order, except the provider userId
     * values have been replaced by Connection instances. If no connection exists between the
     * current user and a given provider user, a null value is returned for that position.
     *
     * @param userId          本地账户用户 Id
     * @param providerUserIds 第三方用户 Id
     * @return the provider user connection map
     */
    MultiValueMap<String, ConnectionData> findConnectionsToUsers(
            String userId, MultiValueMap<String, String> providerUserIds);

    /**
     * 获取 userId 和 providerId 的所以 rank 值最小的绑定信息. Get the "primary" connection the current user. If
     * the user has multiple connections to the provider, this method returns the one with the top
     * rank (or priority). Useful for direct use by application code to obtain a parameterized
     * Connection instance.
     *
     * @param userId     本地账户用户 Id
     * @param providerId the provider id e.g. "qq"
    ...
    /**
     * 绑定. Add a new connection to this repository for the current user. After the connection is
     * added, it can be retrieved later using one of the finders defined in this interface.
     *
    ...
    /**
     * Update a Connection already added to this repository. Merges the field values of the given
     * connection object with the values stored in the repository.
     *
    ...
    /**
     * 解除绑定. Remove all Connections between the current user and the provider from this repository.
    ...
    /**
     * 解除绑定. Remove a single Connection for the current user from this repository. Does nothing if
     ...
     */
    ConnectionData getConnection(String userId, ConnectionKey connectionKey)
     ...
     * @return connection data list
     */
    List<ConnectionData> findConnectionsToUsers(
            MapSqlParameterSource parameters, String providerUsersCriteriaSql, String userId);

    /**
     * 根据 {@code AuthTokenPo#getId()} 更新 {@link ConnectionData}.<br> 注意: 使用这个接口更新会使 spring cache
     * 缓存更新延迟, 出现缓存强一致性问题, 这接口目的用于 refreshToken 的定时任务, 更新 时间一般在凌晨 1-5 点.
     *
    ...
    /**
     * 根据 tokenId 查找 {@link ConnectionData}<br> 注意: 这里不做 spring cache 缓存处理, 这个接口主要用于 refreshToken
     * 的定时任务, 只调用一次, 缓存无意义
     *
