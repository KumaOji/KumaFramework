...
/**
 * 默认的第三方授权登录时自动注册处理器。<br> {@link #signUp(AuthUser, String, String)} 功能：第三方登录自动注册时, 根据 第三方的 authUser
 * 注册为本地账户的用户, 用户名的生成规则由 {@link UmsUserDetailsService#generateUsernames(AuthUser)} 定义, 默认规则为:
 ...
 * @see com.kuma.boot.security.spring.authentication.login.social.justauth.service.ConnectionService
 */
public class DefaultConnectionService implements com.kuma.boot.security.spring.authentication.login.social.justauth.service.ConnectionService {
    ...
    private final com.kuma.boot.security.spring.authentication.login.social.justauth.service.Auth2StateCoder auth2StateCoder;
    ...
    public DefaultConnectionService(
            ...
            com.kuma.boot.security.spring.authentication.login.social.justauth.service.Auth2StateCoder auth2StateCoder) {
    ...
    public UserDetails signUp(
            @NonNull AuthUser authUser, @NonNull String providerId, @NonNull String encodeState)
        ...
        try {
            ...
            if (username == null) {
                throw new RegisterUserFailureException(
                        ErrorCodeEnum.USERNAME_USED, authUser.getUsername());
            }

            // 解密 encodeState
            // https://gitee.com/pcore/just-auth-spring-security-starter/issues/I22JC7
            String decodeState;
            ...
            UserDetails userDetails =
                    userDetailsService.registerUser(
                            authUser, username, defaultAuthorities, decodeState);
            ...
            LogUtils.error(
                    String.format(
                            ...
                            e.getMessage(), username, JacksonUtils.toJson(authUser)),
                    e);
    ...
    public void updateUserConnectionAndAuthToken(
            @NonNull AuthUser authUser, @NonNull ConnectionData data)
        ...
        try {
            ...
            AuthTokenPo authToken =
                    JustAuthUtil.getAuthTokenPo(token, data.getProviderId(), this.timeout);
            ...
            connectionData =
                    JustAuthUtil.getConnectionData(
                            data.getProviderId(), authUser, data.getUserId(), authToken);
            ...
            LogUtils.error("更新第三方用户信息异常: " + e.getMessage());
            throw new UpdateConnectionException(
                    ErrorCodeEnum.UPDATE_CONNECTION_DATA_FAILURE, connectionData, e);
        }
    }
    ...
    public void unbinding(
            @NonNull String userId, @NonNull String providerId, @NonNull String providerUserId) {
        ...
        // 解除绑定(第三方)
        usersConnectionRepository.removeConnection(
                userId, new ConnectionKey(providerId, providerUserId));
    }
    ...
    public List<ConnectionData> findConnectionByProviderIdAndProviderUserId(
        ...
        return usersConnectionRepository.findConnectionByProviderIdAndProviderUserId(
    ...
    public MultiValueMap<String, ConnectionDto> listAllConnections(@NonNull String userId) {
        MultiValueMap<String, ConnectionData> allConnections =
                usersConnectionRepository.findAllConnections(userId);
        Set<Map.Entry<String, List<ConnectionData>>> entrySet = allConnections.entrySet();
        MultiValueMap<String, ConnectionDto> connectionMap =
                new LinkedMultiValueMap<>(allConnections.size());
        ...
        try {
            ...
            if (authToken.getId() == null) {
                try {
                    ...
                    msg =
                            String.format(
                                    "第三方授权登录自动注册时: 本地账户注册成功, %s, 添加第三方授权登录信息失败: %s",
                                    userDetails, JacksonUtils.toJson(authUser));
                    LogUtils.error(msg, e);
                    throw new RegisterUserFailureException(
                            ErrorCodeEnum.USER_REGISTER_OAUTH2_FAILURE,
                            ex,
                ...
                try {
                    ...
                    msg =
                            String.format(
                                    "第三方授权登录自动注册时: 本地账户注册成功, %s, 添加第三方授权登录信息失败: %s, 但 AuthToken 能成功执行 sql, 但已回滚: "
                                            + "%s",
                                    userDetails,
                                    authUser.getRawUserInfo(),
                                    JacksonUtils.toJson(authToken));
                    LogUtils.error(msg, e);
                    throw new RegisterUserFailureException(
    ...
    private void addConnectionData(
        ...
        ConnectionData connectionData =
                JustAuthUtil.getConnectionData(providerId, authUser, userId, authToken);
