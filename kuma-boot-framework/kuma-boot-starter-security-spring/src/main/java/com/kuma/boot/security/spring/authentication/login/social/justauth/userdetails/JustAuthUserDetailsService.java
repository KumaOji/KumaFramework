...
/**
 * 用户名密码、手机短信登录、第三方授权登录及自动注册、用户注册服务：<br><br> 1. 用于用户名密码登录与手机短信登录逻辑。<br><br> 2.
 * 用于用户名密码注册与手机短信注册逻辑。<br><br> 3. 用于第三方登录注册逻辑。<br><br>
 */
public interface JustAuthUserDetailsService extends com.kuma.boot.security.spring.authentication.login.social.justauth.userdetails.JustAuthUserDetailsRegisterService {

    /**
     * 用于第三方登录时查询服务, userId 为本地账户的 userId
     *
     * @param userId userId 为本地账户的 userId (必须具有唯一性, username 支持唯一索引, 也可以是 username)
     * @return the UserDetails requested, 注意 {@link UserDetails#getUsername()} 中的 username 即视为
     * userId
    ...
    /**
     * 在本地账户中检查是否存在 usernames, usernames 为本地账户的 usernames.<br> 注意: 如果在你本地账户用户名允许重名, 直接都返回 true(List
     * = [true, true, true])
     *
