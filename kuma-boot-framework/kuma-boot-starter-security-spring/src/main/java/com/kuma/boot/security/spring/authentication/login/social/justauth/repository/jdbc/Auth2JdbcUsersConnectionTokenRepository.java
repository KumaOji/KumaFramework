...
import static com.kuma.boot.security.justauth.justauth.EnableRefresh.YES;
...
public class Auth2JdbcUsersConnectionTokenRepository implements UsersConnectionTokenRepository {
    ...
    //    @CachePut(cacheNames = RedisCacheAutoConfiguration.USER_CONNECTION_CACHE_NAME, key =
    // "'s:token:' +
    // #result.id")
    @Override
    @NonNull
    public AuthTokenPo saveAuthToken(@NonNull AuthTokenPo authToken) throws DataAccessException {
        jdbcTemplate.update(
                "INSERT INTO "
                        + authTokenTableName
                        + "(`enableRefresh` ,`providerId`, `accessToken`, "
    ...
    public Long getMaxTokenId() throws IncorrectResultSizeDataAccessException {
        Long max =
                jdbcTemplate.queryForObject(
                        "SELECT MAX(`id`) FROM `" + authTokenTableName + "`", Long.class);
        if (isNull(max)) {
            return 1L;
        }
        return max;
    ...
    public List<AuthTokenPo> findAuthTokenByExpireTimeAndBetweenId(
            @NonNull Long expiredTime, @NonNull Long startId, @NonNull Long endId)
    ...
    @Transactional(
            ...
            propagation = Propagation.REQUIRED)
    @Override
    public void updateEnableRefreshByTokenId(
            @NonNull EnableRefresh enableRefresh, @NonNull Long tokenId)
            throws DataAccessException {
        jdbcTemplate.update(
