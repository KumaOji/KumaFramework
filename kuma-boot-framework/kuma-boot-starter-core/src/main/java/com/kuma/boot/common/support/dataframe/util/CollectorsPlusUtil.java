...
public class CollectorsPlusUtil {
    ...
    public static <T, R extends Number> Collector<T, ?, BigDecimal> summingBigDecimalForNumber(
            NumberFunction<T, R> numberFunction ) {
        BigDecimalFunction<? super T> mapper = getBigDecimalFunction(numberFunction);
        return summingBigDecimal(mapper);
    }

    private static <T, R extends Number> BigDecimalFunction<? super T> getBigDecimalFunction(
            NumberFunction<T, R> numberFunction ) {
        return ( e ) -> {
            ...
            return apply instanceof BigDecimal
                    ? (BigDecimal) apply
                    : new BigDecimal(apply.toString());
        };
    }

    public static <T> Collector<T, ?, BigDecimal> summingBigDecimal(
            BigDecimalFunction<? super T> mapper ) {
    ...
    public static List<Map.Entry<String, BigDecimal>> sortByValueAndReverse(
            List<Map.Entry<String, BigDecimal>> unorderedList ) {
        Collections.sort(
                unorderedList,
                new Comparator<Map.Entry<String, BigDecimal>>() {
                    @Override
                    public int compare(
                            Map.Entry<String, BigDecimal> o1, Map.Entry<String, BigDecimal> o2 ) {
    ...
    public static List<Map.Entry<String, Long>> sortByValueAndReverseForLong(
            List<Map.Entry<String, Long>> unorderedList ) {
        Collections.sort(
                unorderedList,
    ...
    public static <T, R extends Number> Collector<T, ?, BigDecimal> averagingBigDecimal(
            NumberFunction<T, R> mapper, int newScale, int roundingMode ) {
    ...
    public static <T> Collector<T, ?, BigDecimal> averagingBigDecimal(
            BigDecimalFunction<? super T> mapper, int newScale, int roundingMode ) {
        return new CollectorImpl<>(
