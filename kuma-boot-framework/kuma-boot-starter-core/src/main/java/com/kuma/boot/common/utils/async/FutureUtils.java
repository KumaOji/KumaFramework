...
public class FutureUtils {
    ...
    public static <T> CompletableFuture<T> unsupportedOperationFuture() {
        ...
        CompletableFuture<T> completableFuture =
                (CompletableFuture<T>) UNSUPPORTED_OPERATION_FUTURE;
        return completableFuture;
    }

    /**
     * Fakes asynchronous execution by immediately executing the operation and completing
     * the supplied future either normally or exceptionally.
    ...
    /**
     * Retry the given operation the given number of times in case of a failure only when
     * an exception is retryable.
     * @param operation to executed
     * @param retries if the operation failed
                        ...
                        if (throwable != null) {
                            if (throwable instanceof CancellationException) {
                                ...
                                if (!retryPredicate.test(throwable)) {
                                    resultFuture.completeExceptionally(
                                            new RetryException(
                                                    "Stopped retrying the operation because the"
     ...
     * @param <T> type of the result
     * @return Future which retries the given operation a given amount of times and delays
     * the retry in case of failures
     */
    ...
    /**
     * Retry the given operation with the given delay in between successful completions
     * where the result does not match a given predicate.
     * @param operation to retry
     ...
     * @param <T> type of the result
     * @return Future which retries the given operation a given amount of times and delays
     * the retry in case the predicate isn't matched
     */
                    ...
                    (t, throwable) -> {
                        if (throwable != null) {
                            ...
                            if (acceptancePredicate.test(t)) {
                                ...
                                resultFuture.completeExceptionally(
                                        new RetryException(
                                                "Could not satisfy the predicate within the allowed"
                                                        + " time."));
                            }
                        }
                    });
     ...
     * {@link #retry(Supplier, int, Executor)} operation fails.
     */
     ...
     * @param timeout after which the given future is timed out
     * @param timeUnit time unit of the timeout
     * @param timeoutFailExecutor executor that will complete the future exceptionally
    ...
    /**
     * Run the given {@code RunnableFuture} if it is not done, and then retrieves its
     * result.
    ...
    /**
     * Run the given action after the completion of the given future. The given future can
     * be completed normally or exceptionally. In case of an exceptional completion the,
     * the action's exception will be added to the initial exception.
     * @param future to wait for its completion
     * @param runnable action which is triggered after the future's completion
     * @return Future which is completed after the action has completed. This future can
     * contain an exception, if an error occurred in the given future or action.
     */
    ...
    /**
     * Run the given asynchronous action after the completion of the given future. The
     * given future can be completed normally or exceptionally. In case of an exceptional
     * completion, the asynchronous action's exception will be added to the initial
     * exception.
     * @param future to wait for its completion
     * @param composedAction asynchronous action which is triggered after the future's
     * completion
     * @return Future which is completed after the asynchronous action has completed. This
     * future can contain an exception if an error occurred in the given future or
     * asynchronous action.
     */
    ...
    /**
     * Creates a future that is complete once multiple other futures completed. The future
     * fails (completes exceptionally) once one of the futures in the conjunction fails.
     * Upon successful completion, the future returns the collection of the futures'
     * results.
     *
     * <p>
     ...
     * @param futures The futures that make up the conjunction. No null entries are
     * allowed.
     * @return The ConjunctFuture that completes once all given futures are complete (or
     * one fails).
     */
    ...
    /**
     * Creates a future that is complete once all of the given futures have completed. The
     * future fails (completes exceptionally) once one of the given futures fails.
     *
     * <p>
     * The ConjunctFuture gives access to how many Futures have already completed
     * successfully, via {@link ConjunctFuture#getNumFuturesCompleted()}.
     * @param futures The futures to wait on. No null entries are allowed.
     * @return The WaitingFuture that completes once all given futures are complete (or
     * one fails).
     */
    ...
    /**
     * A future that is complete once multiple other futures completed. The futures are
     * not necessarily of the same type. The ConjunctFuture fails (completes
     * exceptionally) once one of the Futures in the conjunction fails.
     *
     * <p>
     * The advantage of using the ConjunctFuture over chaining all the futures (such as
     * via {@link CompletableFuture#thenCombine(CompletionStage, BiFunction)}) is that
     * ConjunctFuture also tracks how many of the Futures are already complete.
     */
    ...
    /**
     * The implementation of the {@link ConjunctFuture} which returns its Futures' result
     * as a collection.
     */
    private static class ResultConjunctFuture<T> extends ConjunctFuture<Collection<T>> {
        ...
        private void handleCompletedFuture(int index, T value, Throwable throwable) {
            if (throwable != null) {
                ...
                /**
                 * This {@link #results} update itself is not synchronised in any way and
                 * it's fine because:
                 *
                 * <ul>
                 * <li>There is a happens-before relationship for each thread (that is
                 * completing the future) between setting {@link #results} and
                 * incrementing {@link #numCompleted}.
                 * <li>Each thread is updating uniquely different field of the
                 * {@link #results} array.
                 * <li>There is a happens-before relationship between all of the writing
                 * threads and the last one thread (thanks to the {@code
                 *       numCompleted.incrementAndGet() == numTotal} check.
                 * <li>The last thread will be completing the future, so it has
                 * transitively happens-before relationship with all of preceding
                 * updated/writes to {@link #results}.
                 * <li>{@link AtomicInteger#incrementAndGet} is an equivalent of both
                 * volatile read & write
                 * </ul>
                 */
                results[index] = value;
    ...
    /**
     * Implementation of the {@link ConjunctFuture} interface which waits only for the
     * completion of its futures and does not return their values.
     */
    private static final class WaitingConjunctFuture extends ConjunctFuture<Void> {
        ...
        /**
         * Method which increments the atomic completion counter and completes or fails
         * the WaitingFutureImpl.
         */
    ...
    /**
     * Creates a {@link ConjunctFuture} which is only completed after all given futures
     * have completed. Unlike {@link FutureUtils#waitForAll(Collection)}, the resulting
     * future won't be completed directly if one of the given futures is completed
     * exceptionally. Instead, all occurring exception will be collected and combined to a
     * single exception. If at least on exception occurs, then the resulting future will
    ...
    /**
     * {@link ConjunctFuture} implementation which is completed after all the given
     * futures have been completed. Exceptional completions of the input futures will be
     * recorded but it won't trigger the early completion of this future.
     */
    ...
    /**
     * This function takes a {@link CompletableFuture} and a function to apply to this
     * future. If the input future is already done, this function returns
     * {@link CompletableFuture#thenApply(Function)}. Otherwise, the return value is
     * {@link CompletableFuture#thenApplyAsync(Function, Executor)} with the given
     * executor.
     * @param completableFuture the completable future for which we want to apply.
     * @param executor the executor to run the apply function if the future is not yet
     * done.
     * @param applyFun the function to apply.
     * @param <IN> type of the input future.
     * @param <OUT> type of the output future.
     * @return a completable future that is applying the given function to the input
     * future.
     */
    ...
    /**
     * This function takes a {@link CompletableFuture} and a function to compose with this
     * future. If the input future is already done, this function returns
     * {@link CompletableFuture#thenCompose(Function)}. Otherwise, the return value is
     * {@link CompletableFuture#thenComposeAsync(Function, Executor)} with the given
     * executor.
     * @param completableFuture the completable future for which we want to compose.
     * @param executor the executor to run the compose function if the future is not yet
     * done.
     * @param composeFun the function to compose.
     * @param <IN> type of the input future.
     * @param <OUT> type of the output future.
     * @return a completable future that is a composition of the input future and the
     * function.
     */
    ...
    /**
     * This function takes a {@link CompletableFuture} and a bi-consumer to call on
     * completion of this future. If the input future is already done, this function
     * returns {@link CompletableFuture#whenComplete(BiConsumer)}. Otherwise, the return
     * value is {@link CompletableFuture#whenCompleteAsync(BiConsumer, Executor)} with the
     * given executor.
     * @param completableFuture the completable future for which we want to call
     * #whenComplete.
     * @param executor the executor to run the whenComplete function if the future is not
     * yet done.
     * @param whenCompleteFun the bi-consumer function to call when the future is
     * completed.
     * @param <IN> type of the input future.
    ...
    /**
     * This function takes a {@link CompletableFuture} and a consumer to accept the result
     * of this future. If the input future is already done, this function returns
     * {@link CompletableFuture#thenAccept(Consumer)}. Otherwise, the return value is
     * {@link CompletableFuture#thenAcceptAsync(Consumer, Executor)} with the given
     * executor.
     * @param completableFuture the completable future for which we want to call
     * #thenAccept.
     * @param executor the executor to run the thenAccept function if the future is not
     * yet done.
    ...
    /**
     * This function takes a {@link CompletableFuture} and a handler function for the
     * result of this future. If the input future is already done, this function returns
     * {@link CompletableFuture#handle(BiFunction)}. Otherwise, the return value is
     * {@link CompletableFuture#handleAsync(BiFunction, Executor)} with the given
     * executor.
     * @param completableFuture the completable future for which we want to call #handle.
     * @param executor the executor to run the handle function if the future is not yet
     * done.
     ...
     * @throws IllegalStateException Thrown, if future has not completed or it has
    ...
    /**
     * @return the result of completable future, or the defaultValue if it has not yet
     * completed.
     */
    ...
    /**
     * Asserts that the given {@link CompletableFuture} is not completed exceptionally. If
     * the future is completed exceptionally, then it will call the
     * {@link FatalExitExceptionHandler}.
    ...
    /**
     * Checks that the given {@link CompletableFuture} is not completed exceptionally with
     * the specified class. If the future is completed exceptionally with the specific
     * class, then try to recover using a given exception handler. If the exception does
     * not match the specified class, just pass it through to later stages.
     * @param completableFuture to assert for a given exception
     * @param exceptionClass exception class to assert for
     * @param exceptionHandler to call if the future is completed exceptionally with the
     * specific exception
    ...
    /**
     * Checks that the given {@link CompletableFuture} is not completed exceptionally. If
     * the future is completed exceptionally, then it will call the given uncaught
     * exception handler.
     * @param completableFuture to assert for no exceptions
                                ...
                                try {
                                    ...
                                    final RuntimeException errorHandlerException =
                                            new IllegalStateException(
                                                    "An error occurred while executing the error"
                                                            + " handling for a "
    ...
    /**
     * Forwards the value from the source future to the target future using the provided
     * executor.
     * @param source future to forward the value from
     * @param target future to forward the value to
    ...
    /**
     * Throws the causing exception if the given future is completed exceptionally,
    ...
    /**
     * Completes the given future with either the given value or throwable, depending on
     * which parameter is not null.
    ...
    /**
     * Switches the execution context of the given source future. This works for normally
     * and exceptionally completed futures.
