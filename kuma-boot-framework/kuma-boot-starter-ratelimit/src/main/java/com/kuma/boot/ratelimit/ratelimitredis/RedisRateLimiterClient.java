 ...
 */
public class RedisRateLimiterClient implements com.kuma.boot.ratelimit.ratelimitredis.RateLimiterClient {
    ...
    public RedisRateLimiterClient(
            StringRedisTemplate redisTemplate, RedisScript<List<Long>> script, Environment environment) {
    ...
    public boolean isAllowed(String key, long max, long ttl, TimeUnit timeUnit) {
        // redis key
        String redisKeyBuilder = REDIS_KEY_PREFIX + getApplicationName(environment) + CharPool.COLON + key;
