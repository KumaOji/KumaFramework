 ...
 * <p>两种模式的拦截
 *
 * <p>1.rid 是针对每一次请求的 2.key+val 是针对相同参数请求
 *
 * @author kuma
 * @version 2021.9
...
public class IdempotentAspect {
    ...
    @Pointcut("@annotation(com.kuma.boot.idempotent.annotation.Idempotent)")
    public void watchIde() {}

    @Before("watchIde()")
    public void doBefore(JoinPoint joinPoint) throws Exception {
        Idempotent idempotent = AnnotationUtils.getAnnotation(AopUtils.getMethod(joinPoint), Idempotent.class);
            ...
            if (idempotent.ideTypeEnum() == IdempotentTypeEnum.ALL
                ...
                try {
                    if (StringUtils.isNotBlank(rid)) {
                        ...
                        //	10,
                        //	TimeUnit.SECONDS);
                        boolean result = distributedLock.tryLock(REDIS_KEY_PREFIX + rid, 10L, TimeUnit.SECONDS);
                        if (result) {
                ...
                if (StringUtils.isNotBlank(key)) {
                    ...
                    if (StringUtils.isNotBlank(val)) {
                        ...
                        try {
                            ...
                            //	10,
                            //	TimeUnit.SECONDS);
                            boolean result = distributedLock.tryLock(perFix, 10L, TimeUnit.SECONDS);
                            if (result) {
                                String targetName =
