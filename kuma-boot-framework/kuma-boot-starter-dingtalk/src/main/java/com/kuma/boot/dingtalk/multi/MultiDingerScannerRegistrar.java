...
import static com.kuma.boot.dingtalk.enums.ExceptionEnum.MULTIDINGER_ALGORITHM_EXCEPTION;
import static com.kuma.boot.dingtalk.enums.ExceptionEnum.MULTIDINGER_ANNOTATTION_EXCEPTION;
import static com.kuma.boot.dingtalk.enums.MultiDingerConfigContainer.GLOABL_KEY;
 ...
 */
public class MultiDingerScannerRegistrar extends DingerListenersProperty
     ...
     *
     * {<br>
     * key: dingerClassName | {@link MultiDingerConfigContainer#GLOABL_KEY}(key) + {@link
     * DingerConstant#SPOT_SEPERATOR} + {@link AlgorithmHandler}.simpleName<br>
     * value: {@link MultiDingerAlgorithmDefinition}<br>
     * }<br>
     *
     * </blockquote>
     */
    protected static final Map<String, MultiDingerAlgorithmDefinition> MULTIDINGER_ALGORITHM_DEFINITION_MAP =
            new HashMap<>();
    ...
    private void doScanAndRegister(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
        ...
        if (aloneMulti) {
            ...
            for (AnnotationAttributes attributes : value) {
                ...
                LogUtils.debug(
                        "enable {} global multi dinger, and multiDinger handler class={}.", dinger, handler.getName());
                DingerConfigHandler dingerConfigHandler = BeanUtils.instantiateClass(handler);
                registerHandler(registry, dinger, dinger + SPOT_SEPERATOR + GLOABL_KEY, dingerConfigHandler);
                MultiDingerProperty.multiDinger = true;
            }
        }
    }
    ...
    private void multiDingerHandler(BeanDefinitionRegistry registry, List<Class<?>> dingerClasses) {
        ...
        for (Class<?> dingerClass : dingerClasses) {
            if (dingerClass.isAnnotationPresent(MultiHandler.class)) {
                ...
                if (dingerConfigHandler.isInterface()) {
                    LogUtils.warn(
                            "dingerClass={} handler className={} is interface and skip.",
                ...
                registerHandler(registry, dinger, dinger + SPOT_SEPERATOR + key, handler);
        ...
        if (valid == 0) {
            LogUtils.warn("enable global multi dinger but none dinger interface be decorated with" + " @MultiHandler.");
        }
    }
    ...
    private void registerHandler(
            BeanDefinitionRegistry registry, DingerType dinger, String key, DingerConfigHandler dingerConfigHandler) {
        ...
        for (int i = 0; i < dingerConfigs.size(); i++) {
            ...
            if (DingerUtils.isEmpty(dingerConfig.getTokenId())) {
                throw new DingerException(
                        ExceptionEnum.DINGER_CONFIG_HANDLER_EXCEPTION, dingerConfigHandlerClassName, i);
            }
        }
        ...
        if (injectionCnt == 0) {
            ...
            MULTIDINGER_ALGORITHM_DEFINITION_MAP.put(
                    beanName,
                    new MultiDingerAlgorithmDefinition(key, algorithm, dingerConfigs, dingerConfigHandlerClassName));
            mode = AnalysisEnum.SPRING_CONTAINER;
        }
        LogUtils.debug("key={}, algorithm={} analysis through mode {}.", key, algorithm.getSimpleName(), mode);
    }
