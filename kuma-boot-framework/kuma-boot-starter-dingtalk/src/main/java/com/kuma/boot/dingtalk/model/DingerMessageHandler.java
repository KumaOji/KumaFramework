...
public class DingerMessageHandler extends MultiDingerProperty
    ...
    public Map<String, Object> paramsHandler(Method method, DingerDefinition dingerDefinition, Object[] values) {
        ...
        for (int i = 0; i < Objects.requireNonNull(parameters).length; i++) {
            ...
            com.kuma.boot.dingtalk.annatations.Parameter[] panno =
                    parameter.getDeclaredAnnotationsByType(com.kuma.boot.dingtalk.annatations.Parameter.class);
    ...
    DingerType dingerType(Method method) {
        ...
        return dingtalkProperties.getDefaultDinger();
    }
        ...
        if (localDinger == null) {
            ...
            if (multiDinger()) {
                MultiDingerConfig multiDingerConfig =
                        MultiDingerConfigContainer.INSTANCE.get(useDinger, dingerClassName);
                DingerConfig dingerConfig = null;
                if (multiDingerConfig != null) {
                    // 拿到MultiDingerConfig中当前应该使用的DingerConfig
                    dingerConfig = multiDingerConfig
                            .getAlgorithmHandler()
                            .dingerConfig(multiDingerConfig.getDingerConfigs(), dingerMethodDefaultDingerConfig);
                }
            ...
            dingerDefinition = AbstractDingerDefinitionResolver.Container.INSTANCE.get(keyName);
