...
import com.kuma.boot.data.jpa.fenix.consts.SymbolConst;
...
public class FenixConfig {
   ...
   /**
    * Fenix 自定义的 {@link com.kuma.boot.data.jpa.fenix.config.entity.TagHandler} 处理器实现的所在位置，
   ...
   /**
    * 设置是否打印 {@link com.kuma.boot.data.jpa.fenix.bean.SqlInfo} 中的 SQL 语句和参数信息.
    *
   ...
   /**
    * 设置自定义的 {@link com.kuma.boot.data.jpa.fenix.config.entity.TagHandler} 处理器实现的所在位置，
    * 多个用逗号隔开，可以是目录也可以是具体的 java 或 class 文件路径.
    *
   ...
   public String getUnderscoreTransformerPrefix() {
      return underscoreTransformerPrefix;
   }
