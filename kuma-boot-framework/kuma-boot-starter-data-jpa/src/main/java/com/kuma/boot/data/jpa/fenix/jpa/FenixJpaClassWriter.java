...
public class FenixJpaClassWriter {
   ...
   public static synchronized boolean hasDefaultJpaQueryMethodFactoryClass() {
      ...
      try {
         ...
         LogUtils.debug("【Fenix -> 'JPA 版本检测' 提示】检查到你的项目中没有【{}】类，说明你的 Spring Data JPA 版本"
                 + "是 v2.3.0 之前的版本.", JPA_METHOD_FACTORY_NAME);
         hasJpaMethodClass = false;
      } catch (Exception e) {
         if (LogUtils.isDebugEnabled()) {
            LogUtils.debug("【Fenix -> 'JPA 版本检测' 提示】检查你的项目中是否有【{}】类时出错，将默认你的 Spring Data JPA 版本"
                    + "是 v2.3.0 之前的版本.", JPA_METHOD_FACTORY_NAME, e);
         } else {
            LogUtils.error("【Fenix -> 'JPA 版本检测' 错误】检查你的项目中是否有【{}】类时出错，将默认你的 Spring Data JPA 版本"
   ...
   public static synchronized void modify() {
      ...
      if (hasDefaultJpaQueryMethodFactoryClass()) {
         LogUtils.debug("【Fenix 提示】检测到你的 Spring Data JPA 版本是 v2.3.0 及以上，可不用修改 class 来兼容老版本的 JPA.");
         return;
      }

      // 如果已经修改过了，就直接返回即可.
      if (modified.get()) {
         LogUtils.debug("【Fenix 提示】已经修改过了【FenixQueryLookupStrategy.class】中的部分方法，将不再修改.");
         return;
      }

      LogUtils.info("【Fenix 提示】检测到你的 Spring Data JPA 版本较低，为了兼容老版本的 JPA，将修改部分 class 字节码来做兼容。"
      ...
      try {
         ...
         ctMethod.setBody("{return new com.kuma.boot.data.jpa.fenix.jpa.FenixJpaQuery(new org.springframework.data.jpa"
         ...
         LogUtils.error("【Fenix 错误提示】使用 Javassist 修改【FenixQueryLookupStrategy】class 中的代码出错，"
