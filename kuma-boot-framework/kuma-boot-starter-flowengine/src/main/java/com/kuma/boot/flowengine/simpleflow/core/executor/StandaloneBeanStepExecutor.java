...
public class StandaloneBeanStepExecutor implements StepExecutor {
      ...
      try {
         ...
         if (methodName == null || methodName.trim().isEmpty()) {
            ...
            if (beanInstance == null) {
               return StepResult.failure(currentStep.getId(), currentStep.getName(),
                       "Bean not found: " + beanName);
            }

            if (beanInstance instanceof StepHandler) {
               // 如果Bean实现了StepHandler接口，使用execute方法
               LogUtils.info("Bean '{}' implements StepHandler interface, using default execute method", beanName);
               try {
                  StepResult result = ((StepHandler) beanInstance).execute(context);
                  LogUtils.info("Successfully executed default method: {}.execute", beanName);
                  return result;
               } catch (Exception e) {
                  LogUtils.error("Failed to execute default method: {}.execute", beanName, e);
                  return StepResult.failure(currentStep.getId(), currentStep.getName(), e);
               }
            } else {
               // 如果Bean没有实现StepHandler接口，尝试查找execute方法
               methodName = "execute";
               LogUtils.info("Bean '{}' does not implement StepHandler interface, trying default method: execute",
                       beanName);
            }
         }
         ...
         Object beanInstance = StandaloneStepExecutorRegistry.getInstance().getBean(beanName);
         if (beanInstance == null) {
            LogUtils.error("Bean not found: {}", beanName);
            return StepResult.failure(currentStep.getId(), currentStep.getName(), "Bean not found: " + beanName);
         }

         LogUtils.info("Found bean instance: {} ({})", beanName, beanInstance.getClass().getName());
         LogUtils.info("About to invoke method: {} on bean: {}", methodName, beanName);

         // 查找并执行方法
         Object result = invokeMethod(beanInstance, methodName, context);

         LogUtils.info("Successfully executed method: {}.{}, result: {}", beanName, methodName, result);
         return StepResult.success(currentStep.getId(), currentStep.getName());

      } catch (Exception e) {
         LogUtils.error("Failed to execute bean step", e);
   ...
   private Object invokeMethod(Object beanInstance, String methodName, FlowContext context) throws Exception {
      ...
      if (beanInstance instanceof StepHandler) {
         LogUtils.debug("Bean implements StepHandler interface, using execute method");
      ...
      if (methodName == null || methodName.trim().isEmpty()) {
         if (beanInstance instanceof StepHandler) {
            ...
            throw new IllegalArgumentException(
                    "Method name is required for beans that don't implement StepHandler interface");
         }
      }
      ...
      // 执行方法
      LogUtils.info("Invoking method: {} on bean: {} with args: {}", methodName,
              beanInstance.getClass().getSimpleName(), args);
      targetMethod.setAccessible(true);
      Object result = targetMethod.invoke(beanInstance, args);
      LogUtils.info("Method {} executed successfully, result: {}", methodName, result);
      return result;
   }
   ...
   private Object convertToType(Object value, Class<?> targetType) {
      ...
      try {
         ...
         LogUtils.warn("Failed to convert value '{}' to type {}, using default value", value, targetType.getName());
         return getDefaultValue(targetType);
      }
