...
public class StandaloneConditionalStepExecutor implements StepExecutor {
      ...
      try {
         ...
         if (conditionalConfig == null) {
            return StepResult.failure(currentStep.getId(), currentStep.getName(),
                    "Conditional configuration is required");
         }

         // 评估条件表达式
         boolean conditionResult = evaluateCondition(condition, context);
         LogUtils.info("Condition '{}' evaluated to: {}", condition, conditionResult);
         ...
         if (stepsToExecute == null || stepsToExecute.isEmpty()) {
            LogUtils.info("No steps to execute for condition result: {}", conditionResult);
         ...
         LogUtils.error("Error executing conditional step", e);
   ...
   private boolean evaluateCondition(String condition, FlowContext context) {
      ...
      try {
         ...
         LogUtils.warn("Failed to evaluate condition '{}', defaulting to false", condition, e);
         return false;
      }
   }
   ...
   private StepResult executeStep( StepDefinition step, FlowContext context) {
      try {
         ...
         LogUtils.error("Failed to execute step: {}", step.getId(), e);
