...
public class JexlExpressionEngine implements ExpressionEngine {
   ...
   public Object evaluate(String expression, Map<String, Object> context) throws ExpressionException {
      ...
      try {
         ...
         LogUtils.debug("Evaluated expression '{}' with result: {}", expression, result);

         return result;
      } catch (JexlException e) {
         LogUtils.error("Failed to evaluate expression: {}", expression, e);
         throw new ExpressionException("Failed to evaluate expression: " + e.getMessage(), expression, e);
      } catch (Exception e) {
         LogUtils.error("Unexpected error evaluating expression: {}", expression, e);
         throw new ExpressionException("Unexpected error: " + e.getMessage(), expression, e);
      }
   }

   @Override
   public <T> T evaluate(String expression, Map<String, Object> context, Class<T> expectedType)
   ...
   public boolean isValidExpression(String expression) {
      ...
      try {
         ...
         LogUtils.debug("Invalid expression: {}", expression, e);
         return false;
      }
   }
   ...
   public void clearCache() {
      ...
      LogUtils.info("Expression cache cleared");
   }
