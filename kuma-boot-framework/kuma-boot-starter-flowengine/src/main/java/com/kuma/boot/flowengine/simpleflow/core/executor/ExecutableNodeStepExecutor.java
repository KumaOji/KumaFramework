...
/**
 * ExecutableNode步骤执行器 用于执行继承自ExecutableNode的业务节点，避免反射调用提升性能
 */
...
public class ExecutableNodeStepExecutor implements StepExecutor {
      ...
      try {
         ...
         if (nodeInstance == null) {
            return StepResult.failure(currentStep.getId(), currentStep.getName(),
                    "ExecutableNode not found: " + nodeName);
         }

         // 准备执行上下文
         Map<String, Object> executionContext = prepareExecutionContext(context, parameters);

         LogUtils.info("Executing ExecutableNode: {} for step: {}", nodeName, currentStep.getId());

         // 验证节点执行条件
         if (!nodeInstance.validate(executionContext)) {
            return StepResult.failure(currentStep.getId(), currentStep.getName(),
                    "Node validation failed for: " + nodeName);
         }
         ...
         LogUtils.info("ExecutableNode {} executed successfully for step: {}", nodeName, currentStep.getId());
         ...
         LogUtils.error("Error executing ExecutableNode step", e);
   ...
   /**
    * 准备执行上下文 将FlowContext中的数据转换为Map<String, Object>
    */
