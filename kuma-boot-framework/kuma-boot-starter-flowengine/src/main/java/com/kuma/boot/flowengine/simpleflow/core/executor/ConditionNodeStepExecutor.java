...
/**
 * ConditionNode步骤执行器 用于执行继承自ConditionNode的条件判断节点，避免反射调用提升性能
 */
      ...
      try {
         ...
         if (nodeInstance == null) {
            return StepResult.failure(currentStep.getId(), currentStep.getName(),
                    "ConditionNode not found: " + nodeName);
         }

         // 准备执行上下文
         Map<String, Object> executionContext = prepareExecutionContext(context, parameters);

         LogUtils.info("Evaluating ConditionNode: {} for step: {}", nodeName, currentStep.getId());
         ...
         LogUtils.info("ConditionNode {} evaluated to: {} for step: {}", nodeName, conditionResult,
                 currentStep.getId());
         ...
         LogUtils.error("Error executing ConditionNode step", e);
