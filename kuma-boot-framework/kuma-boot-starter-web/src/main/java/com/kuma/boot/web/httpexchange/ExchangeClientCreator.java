...
import static com.kuma.boot.web.httpexchange.HttpExchangeProperties.ClientType.REST_CLIENT;
import static com.kuma.boot.web.httpexchange.HttpExchangeProperties.ClientType.WEB_CLIENT;
import static com.kuma.boot.web.httpexchange.Util.findMatchedConfig;
import static com.kuma.boot.web.httpexchange.Util.hasAnnotation;
...
class ExchangeClientCreator {
    ...
    public <T> T create() {
        ...
        if (isUseHttpExchangeAnnotation) {
            ...
            com.kuma.boot.web.httpexchange.Cache.addClient(result);
            return result;
        }
        ...
        switch (getClientType(channelConfig)) {
            case REST_CLIENT ->
                    builder.exchangeAdapter(RestClientAdapter.create(getClient(
                            new com.kuma.boot.web.httpexchange.Cache.ClientId(channelConfig, REST_CLIENT), () -> buildRestClient(channelConfig))));
            case WEB_CLIENT ->
                    builder.exchangeAdapter(WebClientAdapter.create(
                            getClient(new com.kuma.boot.web.httpexchange.Cache.ClientId(channelConfig, WEB_CLIENT), () -> buildWebClient(channelConfig))));
            default -> throw new IllegalStateException("Unsupported client-type: " + channelConfig.getClientType());
        }
    }

    private static <T> T getClient(com.kuma.boot.web.httpexchange.Cache.ClientId clientId, Supplier<T> supplier) {
        return Boolean.TRUE.equals(clientId.channel().getHttpClientReuseEnabled())
                ? com.kuma.boot.web.httpexchange.Cache.getHttpClient(clientId, supplier)
                : supplier.get();
    }
