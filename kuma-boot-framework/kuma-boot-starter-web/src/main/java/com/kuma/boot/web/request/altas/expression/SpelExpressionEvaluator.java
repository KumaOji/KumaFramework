...
/**
 * SpEL表达式评估器 (增强版) 支持在日志注解中使用SpEL表达式来动态构建日志内容
 * <p>
 * 版本2.0新特性： - 支持中文字符的模板表达式处理 - 智能表达式类型检测 - 多种表达式处理策略 - 更好的错误处理和降级机制
 * <p>
 * 支持的表达式类型： 1. 纯文本: "查询用户信息", "Get_user_information" 2. 纯SpEL: "#{args[0]}", "#{result.success}" 3. 模板混合: "查询用户信息:
 * 用户ID=#{args[0]}"
 * <p>
 * 支持的变量： - args: 方法参数数组 - result: 方法返回值 - exception: 异常对象 - methodName: 方法名称 - className: 类名称 - executionTime: 方法执行时间 -
 * 自定义变量通过LogContext传递
 *
 * @author nemoob
 * @version 2.0.0
...
public class SpelExpressionEvaluator {
    ...
    // 新增：表达式处理策略映射
    private final Map<com.kuma.boot.web.request.altas.expression.ExpressionType, com.kuma.boot.web.request.altas.expression.ExpressionProcessor> processors = new HashMap<>();
    ...
    private void initializeProcessors() {
        processors.put(
                com.kuma.boot.web.request.altas.expression.ExpressionType.PLAIN_TEXT, new com.kuma.boot.web.request.altas.expression.PlainTextProcessor());
        processors.put(com.kuma.boot.web.request.altas.expression.ExpressionType.PURE_SPEL, new com.kuma.boot.web.request.altas.expression.PureSpelProcessor(this, failSafe));
        processors.put(com.kuma.boot.web.request.altas.expression.ExpressionType.TEMPLATE, new TemplateProcessor(this, failSafe));
    }

    /**
     * 评估SpEL表达式 (增强版) 支持中文字符和多种表达式类型
     *
    ...
    public String evaluateExpression(String expressionString, LogContext logContext) {
        ...
        try {
            // 检测表达式类型
            com.kuma.boot.web.request.altas.expression.ExpressionType type = com.kuma.boot.web.request.altas.expression.ExpressionTypeDetector.detectType(expressionString);
            LogUtils.debug("表达式类型检测: {} -> {}, LogContext.args: {}",
                    expressionString, type,
                    logContext != null && logContext.getArgs() != null ?
                            Arrays.toString(logContext.getArgs()) : "null");

            // 选择对应的处理器
            com.kuma.boot.web.request.altas.expression.ExpressionProcessor processor = processors.get(type);
            if (processor == null) {
            ...
            LogUtils.debug("Expression processing result: {} -> {}", expressionString, result);

            return result;

        } catch (Exception e) {
            String errorMsg = "SpEL表达式评估失败: " + expressionString;
            LogUtils.warn(errorMsg + ", LogContext state: " + describeLogContextState(logContext), e);
    ...
    /**
     * 评估条件表达式 条件表达式通常是纯SpEL表达式，但也支持模板格式
     *
    ...
    public boolean evaluateCondition(String conditionString, LogContext logContext) {
        ...
        try {
            ...
            try {
                ...
                LogUtils.warn(errorMsg, fallbackException);
    ...
    /**
     * 创建SpEL评估上下文 package-private 供处理器使用
     */
    EvaluationContext createEvaluationContext( LogContext logContext) {
        ...
        if (applicationContext != null) {
            context.setBeanResolver((evaluationContext, beanName) -> {
                try {
                    ...
                    LogUtils.debug("Failed to get bean: {}", beanName, e);
                    return null;
                }
            });
        }
        ...
        if (logContext != null) {
            ...
            synchronized (logContext) {
                ...
                // 记录调试信息
                LogUtils.debug("创建EvaluationContext - method: {}, args: {}, argsLength: {}",
            ...
            LogUtils.warn("LogContext is null, creating EvaluationContext with default values");
    ...
    private Object evaluateWithTimeout(Expression expression, EvaluationContext context) {
        ...
        if (duration > timeoutMs) {
            LogUtils.warn("SpEL expression execution timeout: {}ms > {}ms", duration, timeoutMs);
        }
     ...
     * 测试表达式类型检测(供调试使用)
     */
    public com.kuma.boot.web.request.altas.expression.ExpressionType detectExpressionType(String expression) {
        return com.kuma.boot.web.request.altas.expression.ExpressionTypeDetector.detectType(expression);
    }
    ...
    private void validateLogContextState( LogContext logContext, String expression) {
        if (logContext == null) {
            LogUtils.warn("LogContext is null, expression: {}", expression);
            return;
        }

        if (expression.contains("args") && logContext.getArgs() == null) {
            LogUtils.warn("表达式引用args但LogContext.args为null，表达式: {}, LogContext: {}",
