...
class Paths:
    """
    Centralized path configuration for KumaAgent application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. KUMA_AGENT_HOME environment variable
        3. Local dev fallback: cwd/.kuma_agent  (when cwd is the project dir)
        4. Default: $HOME/.kuma_agent
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set KUMA_AGENT_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("KUMA_AGENT_HOST_BASE_DIR"):
            return Path(env)
