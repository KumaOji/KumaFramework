...
from kuma_agent.config.memory_config import load_memory_config_from_dict
from kuma_agent.config.model_config import ModelConfig
from kuma_agent.config.sandbox_config import SandboxConfig
from kuma_agent.config.skills_config import SkillsConfig
from kuma_agent.config.subagents_config import load_subagents_config_from_dict
from kuma_agent.config.summarization_config import load_summarization_config_from_dict
from kuma_agent.config.title_config import load_title_config_from_dict
from kuma_agent.config.tool_config import ToolConfig, ToolGroupConfig
from kuma_agent.config.tool_search_config import ToolSearchConfig, load_tool_search_config_from_dict
...
class AppConfig(BaseModel):
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `KUMA_AGENT_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("KUMA_AGENT_CONFIG_PATH"):
            path = Path(os.getenv("KUMA_AGENT_CONFIG_PATH"))
            if not Path.exists(path):
                raise FileNotFoundError(f"Config file specified by environment variable `KUMA_AGENT_CONFIG_PATH` not found at {path}")
            return path
        else:
...
def get_app_config() -> AppConfig:
    ...
    # Reuse the cached path when available to avoid calling os.getcwd()
    # in an async context (which blockbuster flags as a blocking call).
    resolved_path = _app_config_path if _app_config_path is not None else AppConfig.resolve_config_path()
