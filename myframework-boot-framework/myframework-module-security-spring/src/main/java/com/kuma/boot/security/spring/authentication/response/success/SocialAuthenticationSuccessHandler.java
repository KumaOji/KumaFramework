...
public class SocialAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(
        ...
        // clientRegistrationRepository.findByRegistrationId(oauth2Auth.getAuthorizedClientRegistrationId()).getProviderDetails().getAuthorizationUri();
        ...
        //			// 绑定
        //			userToAuthDao.save(new
        // UserToAuthPO().setType(type).setIdentity(identity).setUserId(username).setTime(LocalDateTime.now().toString()));
        //
        ...
        //			session.setAttribute(WebAttributes.THIRD_AUTHENTICATION,
        ...
        ResponseUtils.success(
                response,
                jwtTokenGenerator.socialTokenResponse((OAuth2User) authentication.getPrincipal()));
    }
}
