...
/**
 * 用户名密码注册、手机短信登录与 OAuth 登录的用户注册接口.<br><br> 推荐通过继承来实现 {@link com.kuma.boot.security.spring.authentication.login.social.justauth.service.UmsUserDetailsService} 此接口的功能
 *
 * @author YongWu zheng
...
public interface UserDetailsRegisterService {
     ...
     * @throws RegisterUserFailureException 用户注册失败
     */
    default UserDetails registerUser(ServletWebRequest request)
            throws RegisterUserFailureException {
        throw new RegisterUserFailureException(ErrorCodeEnum.USER_REGISTER_FAILURE, null);
    }

    /**
     * 第三方第一次登录成功后注册接口, <br> 默认方法直接抛出 {@link RegisterUserFailureException}.<br>
    ...
    /**
     * 第三方第一次登录成功后注册接口, 增加 OAuth2 过程中的 state 解密后的字符串信息 decoderState, 以便用户在 OAuth2 流程中添加自定义的信息, 与
     * {@link com.kuma.boot.security.spring.authentication.login.social.justauth.service.Auth2StateCoder} 配合使用.<br> 默认方法直接抛出 {@link RegisterUserFailureException}.<br>
