...
public class Auth2LoginAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    /**
     * Constructs an {@code Auth2LoginAuthenticationFilter} using the provided parameters.
     *
     * @param filterProcessesUrl          the {@code URI} where this {@code Filter} will process the
     *                                    authentication requests, not null
     * @param signUpUrl                   第三方授权登录后如未注册用户不支持自动注册功能, 则跳转到此 url 进行注册逻辑, 此 url
     *                                    必须开发者自己实现
    ...
    public Authentication attemptAuthentication(
        ...
        MultiValueMap<String, String> params =
                com.kuma.boot.security.spring.authentication.login.social.justauth.filter.login.Auth2AuthorizationResponseUtils.toMultiMap(request.getParameterMap());
        if (!com.kuma.boot.security.spring.authentication.login.social.justauth.filter.login.Auth2AuthorizationResponseUtils.isAuthorizationResponse(params)) {
    ...
    protected void successfulAuthentication(
        ...
        if (principal instanceof TemporaryUser temporaryUser
                && StringUtils.hasText(this.signUpUrl)) {
                ...
                try (RedisConnection connection = redisConnectionFactory.getConnection()) {
                    connection
                            ...
                            .set(
                                    ...
                                    JacksonUtils.toJson(temporaryUser)
    ...
    protected void setDetails(
            HttpServletRequest request, JustAuthLoginAuthenticationToken authRequest) {
