...
public class RemoteChangeUserStatusListener
    ...
    public void onApplicationEvent(RemoteChangeUserStatusEvent event) {
        ...
        if (ObjectUtils.isNotEmpty(data)) {
            KmcUserStatus kmcUserStatus = JacksonUtils.toObject(data, KmcUserStatus.class);
            if (ObjectUtils.isNotEmpty(kmcUserStatus)) {
                //                DataItemStatus dataItemStatus =
                // DataItemStatus.valueOf(userStatus.getStatus());
                //                if (ObjectUtils.isNotEmpty(dataItemStatus)) {
                //                    sysUserService.changeStatus(userStatus.getUserId(),
                // dataItemStatus);
                //                }
            }
        }
