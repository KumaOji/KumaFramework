...
/**
 * An implementation of an {@link AuthenticationProvider} for OAuth 2.0 Login, which leverages the
 * OAuth 2.0 Authorization Code Grant Flow.
 * <p>
 * This {@link AuthenticationProvider} is responsible for authenticating an Authorization Code
 * credential with the Authorization Server's Token Endpoint and if valid, exchanging it for an
 * Access Token credential.
 * <p>
 * It will also obtain the user attributes of the End-User (Resource Owner) from the UserInfo
 * Endpoint using an {@link Auth2UserService}, which will create a {@code Principal} in the form of
 * an {@link AuthUser}. The {@code AuthUser} is then associated to the
 * {@link JustAuthLoginAuthenticationToken} to complete the authentication.
 *
 * @author Joe Grandja
 * @author YongWu zheng
 * @see com.kuma.boot.security.spring.authentication.login.social.justauth.JustAuthAuthenticationToken
 ...
 * @see <a target="_blank" href= "https://tools.ietf.org/html/rfc6749#section-4.1.3">Section 4.1.3
 * Access Token Request</a>
 * @see <a target="_blank" href= "https://tools.ietf.org/html/rfc6749#section-4.1.4">Section 4.1.4
...
public class JustAuthLoginAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication)
        ...
        JustAuthLoginAuthenticationToken loginToken =
                (JustAuthLoginAuthenticationToken) authentication;
        ...
        List<ConnectionData> connectionDataList =
                connectionService.findConnectionByProviderIdAndProviderUserId(
                ...
                try {
                    ...
                    AbstractOAuth2TokenAuthenticationToken<OAuth2AccessToken>
                            ...
                            (AbstractOAuth2TokenAuthenticationToken<OAuth2AccessToken>)
                                    authenticationToken;
                    principal =
                            authenticationToUserDetailsConverter.convert(
                                    auth2AccessTokenAbstractOAuth2TokenAuthenticationToken);
                } catch (IllegalArgumentException e) {
                    throw new InternalAuthenticationServiceException(
                            "AbstractOAuth2TokenAuthenticationToken convert to UserDetails error",
                            e);
                }
            } else {
        ...
        if (CollectionUtils.isEmpty(connectionDataList)) {
            ...
            if (principal == null) {
                ...
                // 不支持自动注册, https://gitee.com/pcore/just-auth-spring-security-starter/issues/I22KP3.
                    ...
                    // 创建临时用户的 userDetails, 再次获取通过
                    // SecurityContextHolder.getContext().getAuthentication().getPrincipal()
                    // @formatter:off
                    userDetails =
                            TemporaryUser.builder()
                                    // username = authUser.getUsername() + "_" + providerId + "_" +
                                    // providerUserId
            ...
            // 本地用户已登录, 绑定
                ...
                throw new InternalAuthenticationServiceException(
                        "principal is TemporaryUser or not UserDetails");
            }
        }
        // 4.2 有第三方登录记录
        else {
        ...
        try {
            ...
            additionalAuthenticationChecks(
                    userDetails, (JustAuthLoginAuthenticationToken) authentication);
        ...
        // 7 创建成功认证 token 并返回
        com.kuma.boot.security.spring.authentication.login.social.justauth.JustAuthAuthenticationToken justAuthAuthenticationToken =
                new com.kuma.boot.security.spring.authentication.login.social.justauth.JustAuthAuthenticationToken(
                        userDetails, userDetails.getAuthorities(), providerId);
     ...
     */
    private void asyncUpdateUserConnectionAndToken(
            AuthUser authUser, ConnectionData connectionData) {
        try {
            // 异步更新第三方授权登录用户信息与 token 信息, 拒绝策略为: CALLER_RUNS
            updateConnectionTaskExecutor.execute(
                    () -> {
                        try {
                            connectionService.updateUserConnectionAndAuthToken(
                                    authUser, connectionData);
                        } catch (Exception e) {
                            String msg =
                                    String.format(
                                            "异步更新第三方授权登录用户信息与 token 信息失败: AuthUser=%s, ConnectionData=%s, error=%s",
                                            JacksonUtils.toJson(authUser),
                                            JacksonUtils.toJson(connectionData),
                                            e.getMessage());
            ...
            LogUtils.error(
                    String.format("异步更新第三方授权登录用户信息与 token 信息失败: %s, 再次同步更新", e.getMessage()), e);
     ...
     * <code>UserDetails</code> for a given authentication request. If custom logic is needed to
     * compare additional
     ...
     *                       <code>UserCache</code> or
     *                       {@link JustAuthUserDetailsService#loadUserByUserId(String)}
     * @param authentication the current request that needs to be authenticated
     * @throws AuthenticationException AuthenticationException if the userDetails could not be
     *                                 validated (generally an
     *                                 <code>AuthenticationServiceException</code>)
     */
    @SuppressWarnings("unused")
    protected void additionalAuthenticationChecks(
            UserDetails userDetails, JustAuthLoginAuthenticationToken authentication)
            throws AuthenticationException {
        // 第三方授权登录, 不需要对密码校验.
    }
    ...
    private class DefaultPreAuthenticationChecks implements UserDetailsChecker {
        ...
        public void check(UserDetails user) {
            if (!user.isAccountNonLocked()) {
                LogUtils.debug("User account is locked");
            ...
            if (!user.isEnabled()) {
                LogUtils.debug("User account is disabled");
            ...
            if (!user.isAccountNonExpired()) {
                LogUtils.debug("User account is expired");
    ...
    private class DefaultPostAuthenticationChecks implements UserDetailsChecker {
        ...
        public void check(UserDetails user) {
            if (!user.isCredentialsNonExpired()) {
                LogUtils.debug("User account credentials have expired");
