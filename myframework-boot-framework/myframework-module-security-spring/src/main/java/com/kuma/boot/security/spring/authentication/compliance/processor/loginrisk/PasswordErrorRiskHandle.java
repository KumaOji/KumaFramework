...
@Component
public class PasswordErrorRiskHandle
extends AbstractLoginHandle {
    ...
    public void filterRisk(List<RiskRule> filter, Map<Integer, RiskRule> ruleMap, UserAccount account) {
        RiskRule passwordRisk;
        if (MapUtil.isNotEmpty(ruleMap) && (passwordRisk = ruleMap.get(1)) != null) {
            ...
            Integer count = 0;
            if (count != null && count >= triggerNumber) {
