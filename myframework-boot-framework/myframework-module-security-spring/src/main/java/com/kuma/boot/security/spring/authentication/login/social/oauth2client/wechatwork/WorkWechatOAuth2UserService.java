...
implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public WorkWechatOAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String code = (String)userRequest.getAdditionalParameters().get("code");
        if (!StringUtils.hasText((String)code)) {
            OAuth2Error oauth2Error = new OAuth2Error(MISSING_CODE_ERROR, "Missing required \"code\" attribute in UserInfoEndpoint for Client Registration: " + registrationId, null);
            throw new OAuth2AuthenticationException(oauth2Error, oauth2Error.toString());
        }
        return (WorkWechatOAuth2User)this.getResponse(userRequest).getBody();
    }
    ...
    private ResponseEntity<WorkWechatOAuth2User> getResponse(OAuth2UserRequest userRequest) {
        ...
        try {
            ...
            queryParams.add((Object)"code", (Object)String.valueOf(userRequest.getAdditionalParameters().get("code")));
    ...
    extends AbstractHttpMessageConverter<WorkWechatOAuth2User> {
        ...
        protected WorkWechatOAuth2User readInternal(Class<? extends WorkWechatOAuth2User> clazz, HttpInputMessage inputMessage) throws HttpMessageNotReadableException {
            try {
                return (WorkWechatOAuth2User)this.jsonMessageConverter.read(OAUTH2_USER_OBJECT.getType(), null, inputMessage);
            }
            catch (Exception ex) {
        ...
        protected void writeInternal(WorkWechatOAuth2User tokenResponse, HttpOutputMessage outputMessage) throws HttpMessageNotWritableException {
        }
