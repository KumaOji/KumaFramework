...
public class RepositoryProperties {
    ...
    /**
     * 查询户 authTokenTableName 在数据库中是否存在的语句。 <br><br> 注意： sql 语句中的 %s 必须写上，且 %s 的顺序必须与后面的字段名称所对应的含义对应
     * :<br><br> authTokenTableName、<br><br> database
     */
    ...
    /**
     * 创建 authTokenTableName 的建表语句。 <br><br> 注意： sql 语句中的 %s 必须写上，且 %s 的顺序必须与后面的字段名称所对应的含义对应
     * :<br><br> authTokenTableName、<br><br>
     */
    private String createAuthTokenTableSql =
            "CREATE TABLE `%s` (\n"
    ...
    /**
     * 第三方登录用户数据库用户表 rank 字段名称， 默认为 `rank`。<br><br> 注意：因为 MySQL 8.0 版本 rank 是个关键字。一定要用 ` 包裹。
     */
     ...
     * 如果需要，请实现 {@link UsersConnectionRepositoryFactory}，可以参考
     * {@link Auth2JdbcUsersConnectionRepositoryFactory}。<br><br> 注意： sql 语句中的 %s 必须写上，且 %s
     * 的顺序必须与后面的字段名称所对应的含义对应 :<br><br> userConnectionTableName、<br><br> userIdColumnName、<br><br>
     * providerIdColumnName、<br><br> providerUserIdColumnName、<br><br> rankColumnName、<br><br>
     * displayNameColumnName、<br><br> profileUrlColumnName、<br><br> imageUrlColumnName、<br><br>
     * accessTokenColumnName、<br><br> tokenIdColumnName、<br><br> refreshTokenColumnName、<br><br>
     * expireTimeColumnName、<br><br> userIdColumnName、<br><br> providerIdColumnName、<br><br>
     * providerUserIdColumnName、<br><br> userIdColumnName、<br><br> providerIdColumnName、<br><br>
     * rankColumnName、<br><br> providerIdColumnName、<br><br> providerUserIdColumnName、<br><br>
    ...
    public String getQueryUserConnectionTableExistSql(String databaseName) {
        ...
        return String.format(
                queryUserConnectionTableExistSql, databaseName, userConnectionTableName);
    }
     ...
     * userConnectionTableName
     */
    private String selectFromUserConnectionSql =
    ...
    /**
     * 第三方登录用户数据库用户表更新语句。 "update %s set %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?
     * where %s = ? and %s = ? and %s = ?"<br><br> 注意： sql 语句中的 %s 必须写上，问号必须与指定的 %s 相对应, %s按顺序会用对应的
     * :<br><br> userConnectionTableName、<br><br> displayNameColumnName、<br><br>
     * profileUrlColumnName、<br><br> imageUrlColumnName、<br><br> accessTokenColumnName、<br><br>
     * tokenIdColumnName、<br><br> refreshTokenColumnName、<br><br> expireTimeColumnName、<br><br>
     * userIdColumnName、<br><br> providerIdColumnName、<br><br> providerUserIdColumnName
     */
    ...
    /**
     * 第三方登录用户数据库用户表添加用户语句。 <br><br> 注意： sql 语句中的 %s 必须写上，问号必须与指定的 %s 相对应, %s按顺序会用对应的 :<br><br>
     * userConnectionTableName、<br><br> userIdColumnName、<br><br> providerIdColumnName、<br><br>
     * providerUserIdColumnName、<br><br> rankColumnName、<br><br> displayNameColumnName、<br><br>
     * profileUrlColumnName、<br><br> imageUrlColumnName、<br><br> accessTokenColumnName、<br><br>
     * tokenIdColumnName、<br><br> refreshTokenColumnName、<br><br> expireTimeColumnName
     */
    ...
    /**
     * 第三方登录用户数据库用户表查询添加用户时的所需 rank 的值。 <br><br> 注意： sql 语句中的 %s 必须写上，问号必须与指定的 %s 相对应, %s按顺序会用对应的
     * :<br><br> rankColumnName、<br><br> rankColumnName、<br><br> userConnectionTableName、<br><br>
     * userIdColumnName、<br><br> providerIdColumnName
     */
    private String addConnectionQueryForRankSql =
            "select coalesce(max(%s) + 1, 1) as %s from %s where %s = ? and %s = ?";

    /**
     * 第三方登录用户数据库用户表查询添加用户时的所需 rank 的值。 <br><br> select coalesce(max(%s) + 1, 1) as %s from %s where
     * %s = ? and %s = ? 注意： sql 语句中的 %s 必须写上，问号必须与指定的 %s 相对应, %s按顺序会用对应的 :<br><br>
     * rankColumnName、<br><br> rankColumnName、<br><br> userConnectionTableName、<br><br>
     * userIdColumnName、<br><br> providerIdColumnName
     */
    ...
    /**
     * 第三方登录用户数据库用户表根据 userId 与 providerId 删除多个用户。 <br><br> 注意： sql 语句中的 %s 必须写上，问号必须与指定的 %s 相对应,
     * %s按顺序会用对应的 :<br><br> userConnectionTableName、<br><br> userIdColumnName、<br><br>
     * providerIdColumnName
     */
    ...
    /**
     * 第三方登录用户数据库用户表根据 userId、providerId、providerUserId 删除一个用户。 <br><br> 注意： sql 语句中的 %s
     * 必须写上，问号必须与指定的 %s 相对应, %s按顺序会用对应的 :<br><br> userConnectionTableName、<br><br>
     * userIdColumnName、<br><br> providerIdColumnName、<br><br> providerUserIdColumnName
     */
    ...
    public Boolean getEnableStartUpInitializeTable() {
        return enableStartUpInitializeTable;
    }
    ...
    public String getQueryAuthTokenTableExistSql() {
        return queryAuthTokenTableExistSql;
    }
    ...
    public String getUserConnectionTableName() {
        return userConnectionTableName;
    }
    ...
    public String getProviderUserIdColumnName() {
        return providerUserIdColumnName;
    }
    ...
    public String getQueryUserConnectionTableExistSql() {
        return queryUserConnectionTableExistSql;
    }
