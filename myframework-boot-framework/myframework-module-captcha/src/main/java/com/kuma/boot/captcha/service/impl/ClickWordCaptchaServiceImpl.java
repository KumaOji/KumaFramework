...
extends AbstractCaptchaService {
    ...
    public Captcha get(Captcha captcha) {
        ...
        if (StrUtil.isBlank((CharSequence)imageData.getOriginalImageBase64())) {
    ...
    public Captcha check(Captcha captcha) {
        ...
        try {
            point = JacksonUtils.toList((String)s, Point.class);
            pointJson = ClickWordCaptchaServiceImpl.decrypt(captcha.getPointJson(), ((Point)point.get(0)).getSecretKey());
            point1 = JacksonUtils.toList((String)pointJson, Point.class);
        }
        catch (Exception e) {
        ...
        for (int i = 0; i < point.size(); ++i) {
            if (((Point)point.get((int)i)).x - HAN_ZI_SIZE <= ((Point)point1.get((int)i)).x && ((Point)point1.get((int)i)).x <= ((Point)point.get((int)i)).x + HAN_ZI_SIZE && ((Point)point.get((int)i)).y - HAN_ZI_SIZE <= ((Point)point1.get((int)i)).y && ((Point)point1.get((int)i)).y <= ((Point)point.get((int)i)).y + HAN_ZI_SIZE) continue;
            this.afterValidateFail(captcha);
    ...
    private Captcha getImageData(BufferedImage backgroundImage) {
        ...
        int num = RandomUtils.randomInt((int)1, (int)wordCount);
        Set<String> currentWords = this.getRandomWords(wordCount);
        String secretKey = null;
        if (captchaAesStatus.booleanValue()) {
            secretKey = RandomUtils.randomString((int)16);
        }
        int i = 0;
        for (String word : currentWords) {
            Point point = ClickWordCaptchaServiceImpl.randomWordPoint(width, height, i, wordCount);
            point.setSecretKey(secretKey);
            if (this.isFontColorRandom()) {
                backgroundGraphics.setColor(new Color(RandomUtils.randomInt((int)1, (int)255), RandomUtils.randomInt((int)1, (int)255), RandomUtils.randomInt((int)1, (int)255)));
            } else {
                backgroundGraphics.setColor(Color.BLACK);
            }
            AffineTransform affineTransform = new AffineTransform();
            affineTransform.rotate(Math.toRadians(RandomUtils.randomInt((int)-45, (int)45)), 0.0, 0.0);
    ...
    private Set<String> getRandomWords(int wordCount) {
        ...
        do {
            String t = "" + HAN_ZI.charAt(RandomUtils.randomInt((int)size));
            words.add(t);
    ...
    private static Point randomWordPoint(int imageWidth, int imageHeight, int wordSortIndex, int wordCount) {
        ...
        int x = avgWidth < HAN_ZI_SIZE_HALF ? RandomUtils.randomInt((int)(1 + HAN_ZI_SIZE_HALF), (int)imageWidth) : (wordSortIndex == 0 ? RandomUtils.randomInt((int)(1 + HAN_ZI_SIZE_HALF), (int)(avgWidth * (wordSortIndex + 1) - HAN_ZI_SIZE_HALF)) : RandomUtils.randomInt((int)(avgWidth * wordSortIndex + HAN_ZI_SIZE_HALF), (int)(avgWidth * (wordSortIndex + 1) - HAN_ZI_SIZE_HALF)));
        int y = RandomUtils.randomInt((int)HAN_ZI_SIZE, (int)(imageHeight - HAN_ZI_SIZE));
