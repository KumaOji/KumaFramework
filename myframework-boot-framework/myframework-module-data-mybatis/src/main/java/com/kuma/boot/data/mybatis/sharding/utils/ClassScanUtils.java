...
public class ClassScanUtils {
     ...
     */
    public static Class<?> generateMapperViaMapper(
            Class<?> clz, String dsName, boolean isShardingTable) {
        String name = com.kuma.boot.data.mybatis.sharding.utils.NameUtils.buildClassName(dsName, clz.getCanonicalName());
        return ClassScanUtils.genInterfaceViaInterface(clz, name, isShardingTable);
    }

    private static Class<?> genInterfaceViaInterface(
            Class<?> clz, String name, boolean isShardingTable) {
        ...
        for (Field f : fields) {
            try {
                ...
                LogUtils.warn("genInterfaceViaInterface - gen field failed:{}", f.getName());
            }
        }
        // 生成方法
        ...
        return unloaded.load(clz.getClassLoader(), ClassLoadingStrategy.Default.INJECTION)
                .getLoaded();
    }

    private static DynamicType.Builder<?> genFields(DynamicType.Builder<?> builder, Field f)
            throws IllegalAccessException {
        if (f.getType().equals(Integer.TYPE)) {
            builder =
                    builder.defineField(f.getName(), f.getType(), f.getModifiers())
                            .value((int) f.get(null));
        } else if (f.getType() == char.class) {
            builder =
                    builder.defineField(f.getName(), f.getType(), f.getModifiers())
                            .value((char) f.get(null));
        } else if (f.getType() == short.class) {
            builder =
                    builder.defineField(f.getName(), f.getType(), f.getModifiers())
                            .value((short) f.get(null));
        } else if (f.getType() == byte.class) {
            builder =
                    builder.defineField(f.getName(), f.getType(), f.getModifiers())
                            .value((byte) f.get(null));
        } else if (f.getType().equals(Long.TYPE)) {
            builder =
                    builder.defineField(f.getName(), f.getType(), f.getModifiers())
                            .value((long) f.get(null));
        } else if (f.getType().equals(Float.TYPE)) {
            builder =
                    builder.defineField(f.getName(), f.getType(), f.getModifiers())
                            .value((float) f.get(null));
        } else if (f.getType().equals(Double.TYPE)) {
            builder =
                    builder.defineField(f.getName(), f.getType(), f.getModifiers())
                            .value((double) f.get(null));
                    ...
                    new ByteCodeAppender() {
                        ...
                        public Size apply(
                            ...
                            try {
                                ...
                                LogUtils.warn("field:{} is not defined precisely", f.getName());
                            }
                            if (size != null) {
    ...
    private static DynamicType.Builder<?> genBuilderMethod(
            DynamicType.Builder<?> builder, Method m, boolean isShardingTable) {
        Parameter[] params = m.getParameters();

        Type[] types =
                isShardingTable
                        ? new Type[m.getParameterCount() + 1]
                        : new Type[m.getParameterCount()];
        if (isShardingTable) {
        ...
        DynamicType.Builder.MethodDefinition<?> methodBuilder =
                builder.defineMethod(m.getName(), m.getReturnType(), PUBLIC + ABSTRACT)
                        .withParameters(types)
                        .withoutCode();
        if (isShardingTable) {
            methodBuilder =
                    methodBuilder.annotateParameter(
                            0,
                            AnnotationDescription.Builder.ofType(Param.class)
     ...
     */
    private static void saveClass2Target(Unloaded<?> unloaded) {
