 ...
 * @author wency_cai
 */
public class DefaultSqlParser implements com.kuma.boot.data.mybatis.interceptor.tenant.parser.SqlParser {
    ...
    public String setTenantParameter(String sql, Object paramTenantId) {
        ...
        List<SQLStatement> statementList =
                SQLUtils.parseStatements(sql, tenantInfoHandler.getDbType());
        ...
        if (sqlSelectQuery instanceof SQLSelectQueryBlock) {
            ...
            if (table instanceof SQLExprTableSource) {
                ...
                if (where instanceof SQLInListExpr) {
                    ...
                    isContainsTenantIdCondition =
                            isContainsTenantIdCondition(sqlExprTableSource.getExpr());
                }

                if (!isContainsTenantIdCondition) {
                    SQLExpr newWhereCondition =
                            this.createNewWhereCondition(tableName, where, alias, paramTenantId);
                ...
                if (sqlSelectQueryBlock.getFrom() instanceof SQLExprTableSource) {
                    SQLExprTableSource sqlExprTableSource =
                            (SQLExprTableSource) sqlSelectQueryBlock.getFrom();
                    tableName = sqlExprTableSource.getExpr().toString();
                    ...
                    SQLExprTableSource exprTableSource =
                            lastJoinTableSourceLeadTable(joinTableSource);
                ...
                SQLUnionQueryTableSource sqlUnionQueryTableSource =
                        (SQLUnionQueryTableSource) table;
                // 处理union的查询语句
    ...
    public void processUpdate(SQLUpdateStatement update, Object paramTenantId) {
        ...
        if (sqlTableSource instanceof SQLJoinTableSource) {
            ...
            SQLExpr tenantCondition =
                    getTenantCondition(
                            tableName, right.getAlias(), joinTable.getCondition(), paramTenantId);
            if (tenantCondition != null) {
                joinTable.addCondition(tenantCondition);
            }
        ...
        SQLExpr tenantCondition =
                getTenantCondition(tableName, alias, update.getWhere(), paramTenantId);
        if (tenantCondition != null) {
    ...
    public void processDelete(SQLDeleteStatement delete, Object paramTenantId) {
        ...
        if (tableSource instanceof SQLJoinTableSource
                || delete.getFrom() instanceof SQLJoinTableSource) {
        ...
        SQLExpr tenantCondition =
                getTenantCondition(tableName, alias, delete.getWhere(), paramTenantId);
        if (tenantCondition != null) {
    ...
    private void joinCondition(SQLTableSource sqlTableSource, Object paramTenantId) {
        if (sqlTableSource instanceof SQLJoinTableSource) {
            ...
            SQLExpr tenantCondition =
                    getTenantCondition(
                            tableName,
                            sqlJoinTableSource.getRight().getAlias(),
            ...
            SQLUnionQueryTableSource sqlUnionQueryTableSource =
                    (SQLUnionQueryTableSource) sqlTableSource;
            // 处理union的查询语句
            SQLUnionQuery sqlUnionQuery = sqlUnionQueryTableSource.getUnion();
            unionQuery(sqlUnionQuery, paramTenantId);
        }
    ...
    private SQLExpr createNewWhereCondition(
            String tableName, SQLExpr where, String alias, Object paramTenantId) {
        // 如果是表达式
        if (where != null) {
            ...
            SQLExpr tenantCondition =
                    this.getTenantCondition(tableName, alias, where, paramTenantId);
            if (tenantCondition == null) {
                return where;
            }
     ...
     * @return 返回条件
     */
    private SQLExpr getTenantCondition(
            String tableName, String alias, SQLExpr condition, Object paramTenantId) {
        if (isContainsTenantIdCondition(condition)
                || ignoreTable(tableName)
                || ignoreTableAlias(alias)) {
            return null;
        }
        List<Object> tenantIds = this.getTenantId(paramTenantId);
        if (tenantIds.size() == 1) {
    ...
    private SQLInListExpr conditionIn(String alias, List<Object> tenantIds) {
        if (alias != null) {
            ...
            SQLPropertyExpr sqlPropertyExpr =
                    new SQLPropertyExpr(
                            sqlIdentifierExpr, this.tenantInfoHandler.getTenantIdColumn());
            ...
            return SQLExprUtils.conditionIn(
                    this.tenantInfoHandler.getTenantIdColumn(), tenantIds, null);
        }
    }
    ...
    private boolean isContainsTenantIdCondition(SQLExpr where) {
        ...
        if (left instanceof SQLBinaryOpExpr || left instanceof SQLPropertyExpr) {
            isContainsTenantIdCondition =
                    String.valueOf(left).contains(this.tenantInfoHandler.getTenantIdColumn());
        }
        if (right instanceof SQLBinaryOpExpr) {
            isContainsTenantIdCondition =
                    String.valueOf(right).contains(this.tenantInfoHandler.getTenantIdColumn());
        }

        if (left instanceof SQLIdentifierExpr) {
            isContainsTenantIdCondition =
                    String.valueOf(left).equals(this.tenantInfoHandler.getTenantIdColumn());
        }

        if (right instanceof SQLIdentifierExpr) {
            isContainsTenantIdCondition =
                    String.valueOf(right).equals(this.tenantInfoHandler.getTenantIdColumn());
        }
    ...
    private void unionQuery(SQLUnionQuery sqlUnionQuery, Object paramTenantId) {
        sqlUnionQuery
                .getRelations()
                .forEach(sqlSelectQuery -> processSelectBody(sqlSelectQuery, paramTenantId));
    }
     ...
     */
    private void handlerInsertSelect(
            SQLInsertStatement insert, String tenantIdColumn, Object paramTenantId) {
        // 处理 insert select用法
    ...
    private String getAlias(SQLTableSource sqlTableSource) {
        if (sqlTableSource instanceof SQLJoinTableSource) {
            ...
            if (sqlJoinTableSource.getLeft() instanceof SQLJoinTableSource) {
                ...
                SQLExprTableSource sqlExprTableSource =
                        (SQLExprTableSource) sqlJoinTableSource.getLeft();
                ...
                SQLSubqueryTableSource sqlSubqueryTableSource =
                        (SQLSubqueryTableSource) sqlJoinTableSource.getLeft();
    ...
    public boolean isIgnoreDynamicDatasource() {
        ...
        if (ignoreDynamicDatasource == null
                || ignoreDynamicDatasource.length() == 0
                || isEmpty(ignoreDynamicDatasourceList)) {
            return false;
        }
