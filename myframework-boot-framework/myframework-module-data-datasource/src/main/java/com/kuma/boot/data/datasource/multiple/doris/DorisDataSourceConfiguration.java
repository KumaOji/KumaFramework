...
package com.kuma.boot.data.datasource.multiple.doris;
 ...
 */
@AutoConfiguration
@ConditionalOnProperty(name = "kuma.boot.data.datasource.multiple.doris.enabled", havingValue = "true")
@Import(DorisJdbcBaseDaoImpl.class)
public class DorisDataSourceConfiguration {
    ...
    public DorisDataSourceConfiguration(
    ...
    public DataSource dorisDruidDataSource() {
        ...
        // 配置数据源属性
        datasource.setUrl(extDataSourceProperties.getDoris().getUrl());
        datasource.setUsername(extDataSourceProperties.getDoris().getUsername());
        datasource.setPassword(extDataSourceProperties.getDoris().getPassword());
        datasource.setDriverClassName(extDataSourceProperties.getDoris().getDriverClassName());
    ...
    public JdbcTemplate dorisJdbcTemplate(
            @Qualifier("dorisDruidDataSource") DataSource dataSource ) {
        return new JdbcTemplate(dataSource);
    }

    @Bean(name = "dorisJdbcClient")
    public JdbcClient dorisJdbcClient( @Qualifier("dorisDruidDataSource") DataSource dataSource ) {
