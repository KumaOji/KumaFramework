...
public class MultiBizProxyRegister
    ...
    public void registerBeanDefinitions(
        ...
        for (String scanPackage : scanPackages) {
            ...
            // 遍历每个 bean 注册定义，生产动态代理对象，通过 ImportBeanDefinitionRegistrar 开放的接口registerBeanDefinitions
            // 进行
            ...
            try {
                for (BeanDefinition beanDefinition : beanDefinitions) {
                    ...
                    //                    Map<String,Object>
                    // spiDefineAttrs=annotationMetadataItem.getAnnotationAttributes(SPIDefine.class.getName ());
                    //                    if(spiDefineAttrs ==null || spiDefineAttrs.isEmpty ()){
                    //                        continue;
                    //                    }
                    //                    String routerBeanName=null;
                    //                    if(spiDefineAttrs.get(CommonConsts.SPI_DEFINE_ATTR_NAME)
                    // instanceof String){
                    //                        routerBeanName = (String)
                    // spiDefineAttrs.get(CommonConsts.SPI_DEFINE_ATTR_NAME);
                    //                    }
                    //
                    //                    //get router
                    //                    SPIRouter
                    // spiRouter=beanFactory.getBean(routerBeanName,SPIRouter.class);

                    // 创建beanFactory
                    com.kuma.boot.web.support.ext.ExtensionBeanInterceptor interceptor =
                            this.beanFactory.getBean(com.kuma.boot.web.support.ext.ExtensionBeanInterceptor.class);
                    ...
                    BeanDefinitionBuilder beanDefinitionBuilder =
                            BeanDefinitionBuilder.genericBeanDefinition(spiProxyObject.getClass());
                    beanDefinitionBuilder.addConstructorArgValue(
                            Proxy.getInvocationHandler(spiProxyObject));
                    AbstractBeanDefinition realBeanDefinition =
                            beanDefinitionBuilder.getBeanDefinition();
                    realBeanDefinition.setPrimary(true);

                    // 注册definition到spring容器
                    StringBuilder sb =
                            new StringBuilder().append(clazz.getSimpleName()).append("#Proxy");
    ...
    private ClassPathScanningCandidateComponentProvider getScanner() {
        ...
        // ClassPathScanningCandidateComponentProvider (false, this.envirnoment) {
        //            @Override
        ...
        //                    try {
        //                        Class<?> target =
        // ClassUtils.forName(annotatedBeanDefinition.getMetadata ().getClassName (),
        //                                MultiBizProxyRegister.this.classLoader);
        //                        SPIDefine[] spiDefines = target.getAnnotationsByType
        // (SPIDefine.class);
    ...
    private Set<String> getScanPackages(AnnotationMetadata annotationMetadata) {
        AnnotationAttributes annotationAttributes =
                AnnotationAttributes.fromMap(
                        annotationMetadata.getAnnotationAttributes(RouterBaseScan.class.getName()));
