...
import com.kuma.cloud.job.server.common.grpc.PingServerRpcClient;
...
public class HeartbeatHandler implements RpcHandler {
    ...
    public void handle( Object req, StreamObserver<CommonCausa.Response> responseObserver ) {
        ...
        for (int i = 0; i < RETRY_TIMES; i++) {
            ...
            if (appInfo == null) {
                throw new KmcJobException(appId + " is not registered!");
            }
        ...
        throw new KmcJobException("server elect failed for app " + appId);
    }
    ...
    private boolean checkLocalServer( String currentServer ) {
        // 获取本机的InetAddress对象
        ownerIp = kmcJobServerConfig.getAddress();
