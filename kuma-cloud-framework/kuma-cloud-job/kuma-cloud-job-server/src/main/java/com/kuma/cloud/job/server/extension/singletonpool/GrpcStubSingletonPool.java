...
package com.kuma.cloud.job.server.extension.singletonpool;
...
public class GrpcStubSingletonPool {
    @Autowired static KmcJobServerConfig config;
    ...
    private static ManagedChannel getChannelSingleton(String serverAddress, String type) {
        ...
        switch (type) {
            case RemoteConstant.SERVER:
                port = KmcJobServerConfig.staticServerPort;
                break;
            case RemoteConstant.WORKER:
                port = KmcJobServerConfig.staticWorkerPort;
                break;
            case RemoteConstant.NAMESERVER:
                port = Integer.parseInt(KmcJobServerConfig.staticNameServerAddress.split(":")[1]);
        }
