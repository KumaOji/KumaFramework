...
import com.kuma.cloud.job.client.producer.entity.ResponseFuture;
...
public class MessageSendClient {
                ...
                new InvokeCallback() {
                    ...
                    public void operationComplete( ResponseFuture responseFuture ) {
                        if (responseFuture.isTimeout()) {
                            log.error("[KmcJobProducer] send message timeout");
                            onExceptionImpl(retryTime, curTryTimes, msg);
                        } else if (!responseFuture.isSendResponseOK()) {
                            log.error("[KmcJobProducer] send message error");
                            onExceptionImpl(retryTime, curTryTimes, msg);
                        } else {
                            log.error("[KmcJobProducer] send message error for unknownReason");
                            onExceptionImpl(retryTime, curTryTimes, msg);
                        }
                    }
                });
    ...
    private void onExceptionImpl( int retryTime, AtomicInteger curRetryTimes, MqCausa.Message msg ) {
        ...
        if (curRetryTimes.get() < retryTime) {
            try {
                log.info("[KmcJobProducer] send message retry times:{}", curRetryTimes);
