...
package com.kuma.cloud.job.worker.core.schedule.tracker.task.light;

import com.kuma.cloud.job.common.SystemInstanceResult;
...
import com.kuma.cloud.job.worker.core.schedule.tracker.manager.LightTaskTrackerManager;
import com.kuma.cloud.job.worker.core.schedule.tracker.task.TaskTracker;
import com.kuma.cloud.job.worker.processor.ProcessResult;
import com.kuma.cloud.job.worker.processor.ProcessorBean;
import com.kuma.cloud.job.worker.processor.ProcessorDefinition;
import com.kuma.cloud.job.worker.processor.task.TaskConstant;
import com.kuma.cloud.job.worker.processor.task.TaskContext;
import com.kuma.cloud.job.worker.processor.task.TaskStatus;
...
public class LightTaskTracker extends TaskTracker {
    ...
    public LightTaskTracker(ScheduleCausa.ServerScheduleJobReq req, KmcJobWorkerConfig config) {
        super(req, config);
        try {
            ...
            // 加载 Processor
            processorBean =
                    KmcJobWorkerConfig.getProcessorLoader()
                            .load(
            ...
            //            // 上报任务状态
            //            statusReportScheduledFuture =
            // ExecutorManager.getLightweightTaskStatusCheckExecutor().scheduleWithFixedDelay(new
            // SafeRunnableWrapper(this::checkAndReportStatus), initDelay, delay,
            // TimeUnit.MILLISECONDS);

            // 提交任务到线程池
            processFuture =
                    ExecutorManager.getLightweightTaskExecutorService().submit(this::processTask);
        } catch (Exception e) {
     ...
     */
    public static LightTaskTracker create(
            ScheduleCausa.ServerScheduleJobReq req, KmcJobWorkerConfig config) {
        try {
    ...
    public void destroy() {
        ...
        log.info(
                "[TaskTracker-{}] remove TaskTracker,task status {},start time:{},end time:{},real cost:{},total time:{}",
                instanceId,
                status,
                taskStartTime,
                taskEndTime,
                taskEndTime != null ? taskEndTime - taskStartTime : "unknown",
    ...
    public void stopTask() {
        ...
        if (finished.get()) {
            log.warn(
                    "[TaskTracker-{}] fail to stop task,task is finished!result:{}",
                    instanceId,
                    result);
            return;
        }
        if (!stopFlag.compareAndSet(false, true)) {
            log.warn(
                    "[TaskTracker-{}] task has been mark as stopped,ignore this request!",
                    instanceId);
            return;
        }
        // 当前任务尚未执行
        if (status == TaskStatus.WORKER_RECEIVED) {
            log.warn(
                    "[TaskTracker-{}] task is not started,destroy this taskTracker directly!",
                    instanceId);
            destroy();
            return;
        }
    ...
    private ProcessResult processTask() {
        ...
        do {
            ...
            if (res != null && !res.isSuccess()) {
                ...
                log.warn(
                        ...
                        instanceId,
                        taskContext.getCurrentRetryTimes());
            }
            try {
                ...
                if (timeoutFlag.get()) {
                    res =
                            new ProcessResult(
                                    false,
                                    SystemInstanceResult.INSTANCE_EXECUTE_TIMEOUT_INTERRUPTED);
                } else if (stopFlag.get()) {
                    res =
                            new ProcessResult(
                                    false, SystemInstanceResult.USER_STOP_INSTANCE_INTERRUPTED);
                } else {
            ...
            if (res == null) {
                ...
                && taskContext.getCurrentRetryTimes() < taskContext.getMaxRetryTimes()
                && !timeoutFlag.get()
                && !stopFlag.get());
        ...
        status =
                result.isSuccess()
                        ? TaskStatus.WORKER_PROCESS_SUCCESS
        ...
        //        }
        log.info(
                "[TaskTracker-{}] task complete ! create time:{},queue time:{},use time:{},result:{}",
                instanceId,
                createTime,
                taskStartTime - createTime,
    ...
    //        if (destroyFlag.get()) {
    //            // 已经被销毁，不需要上报状态
    //            log.info("[TaskTracker-{}] has been destroyed,final status is {},needn't to report
    // status!", instanceId, status);
    //            return;
    //        }
    //        TaskTrackerReportInstanceStatusReq reportInstanceStatusReq = new
    ...
    // SystemInstanceResult.USER_STOP_INSTANCE_FORCE_STOP);
    //                        log.warn("[TaskTracker-{}] task need stop, force stop thread {}
    // success!", instanceId, workerThread.getName());
    //                        // 被终止的任务不需要上报状态
    //                        destroy();
    //                        return;
    //                    }
    //                } catch (Exception e) {
    //                    log.warn("[TaskTracker-{}] task need stop,fail to stop thread {}",
    // instanceId, workerThread.getName(), e);
    //                }
    //            }
    //        }
    //        if (finished.get()) {
    ...
    //        log.info("[TaskTracker-{}] report status({}) success,real status is {}", instanceId,
    // reportInstanceStatusReq, status);
    //        TransportUtils.ttReportInstanceStatus(reportInstanceStatusReq,
    // workerRuntime.getServerDiscoveryService().getCurrentServerAddress(),
    // workerRuntime.getTransporter());
    //    }

    //    private void timeoutCheck() {
    //        if (taskStartTime == null || System.currentTimeMillis() - taskStartTime <
    ...
    //            // 超时，仅尝试打断任务
    //            log.warn("[TaskTracker-{}] task
    // timeout,taskStarTime:{},currentTime:{},runningTimeLimit:{}, try to interrupt it.",
    // instanceId, taskStartTime, System.currentTimeMillis(), instanceInfo.getInstanceTimeoutMS());
    //            processFuture.cancel(true);
    //            return;
    //        }
    //        if (finished.get()) {
    //            // 已经成功被打断
    //            log.warn("[TaskTracker-{}] task timeout,taskStarTime:{},endTime:{}, interrupt
    // success.", instanceId, taskStartTime, taskEndTime);
    //            return;
    //        }
    ...
    //                taskEndTime = System.currentTimeMillis();
    //                result = new ProcessResult(false,
    // SystemInstanceResult.INSTANCE_EXECUTE_TIMEOUT_FORCE_STOP);
    //                log.warn("[TaskTracker-{}] task timeout, force stop thread {} success!",
    // instanceId, workerThread.getName());
    //            }
    //        } catch (Exception e) {
    //            log.warn("[TaskTracker-{}] task timeout,fail to stop thread {}", instanceId,
    ...
    //            return result;
    //        }
    //        log.warn("[TaskTracker-{}] task's result is too large({}>{}), a part will be
    // discarded.",
    ...
    //
    //        String allowStopThread =
    // System.getProperty(PowerJobDKey.WORKER_ALLOWED_FORCE_STOP_THREAD);
    //        if (!StringUtils.equalsIgnoreCase(allowStopThread, Boolean.TRUE.toString())) {
    //            log.warn("[TaskTracker-{}] PowerJob not allowed to force stop a thread by config",
    // instanceId);
    //            return false;
    //        }
    //
    //        log.warn("[TaskTracker-{}] fail to interrupt the thread[{}], try to force stop.",
    // instanceId, threadName);
    //        try {
    //            thread.stop();
    //            return true;
    //        } catch (Throwable t) {
    //            log.warn("[TaskTracker-{}] stop thread[{}] failed, msg: {}", instanceId,
