...
package com.kuma.cloud.job.worker.core.discover;

import com.kuma.cloud.job.common.domain.WorkerAppInfo;
 ...
 */
@Slf4j
public class KmcJobServerDiscoverService implements ServerDiscoverService {
    ...
    public KmcJobServerDiscoverService(KmcJobWorkerConfig config) {
        this.config = config;
    }
    ...
    public void heartbeatCheck(ScheduledExecutorService heartbeatCheckExecutor) {
        ...
        heartbeatCheckExecutor.scheduleAtFixedRate(
                () -> {
                    try {
                        setCurrentIpAddress(discovery());
                        log.info("[KmcJobServerDiscovery] jump ip :{}", currentIpAddress);
                    } catch (Exception e) {
                        log.error("[KmcJobServerDiscovery] fail to discovery server!", e);
                    }
                },
                5,
                5,
    ...
    private String discovery() {
        ...
        if (StringUtils.isEmpty(result)) {
            log.warn(
                    "[KmcJobServerDiscovery] can't find any available server, this worker has been quarantined.");
            ...
            // 重置失败次数
            FAILED_COUNT = 0;
            log.debug("[KmcJobServerDiscovery] current server is {}.", result);
            return result;
        }
    }
