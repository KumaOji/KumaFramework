...
package com.kuma.cloud.mq.broker.support.push;

import com.alibaba.fastjson2.JSON;
import com.kuma.cloud.mq.broker.constant.BrokerRespCode;
import com.kuma.cloud.mq.broker.dto.ChannelGroupNameDto;
import com.kuma.cloud.mq.broker.dto.persist.MqMessagePersistPut;
import com.kuma.cloud.mq.broker.support.persist.MqBrokerPersist;
import com.kuma.cloud.mq.common.constant.MessageStatusConst;
...
import com.kuma.cloud.mq.common.dto.resp.MqConsumerResultResp;
 ...
 * @since 2024.05
 */
public class DefaultBrokerPushService implements BrokerPushService {

    private static final Logger log = LoggerFactory.getLogger(DefaultBrokerPushService.class);
        ...
        EXECUTOR_SERVICE.submit(
                new Runnable() {
                    ...
                    public void run() {
                        ...
                        final MqBrokerPersist mqBrokerPersist = context.mqBrokerPersist();
                        final InvokeService invokeService = context.invokeService();
                        ...
                        for (final ChannelGroupNameDto channelGroupNameDto : channelList) {
                            ...
                            final String consumerGroupName =
                                    channelGroupNameDto.getConsumerGroupName();

                            try {
                                mqBrokerPersist.updateStatus(
                                        messageId,
                                        consumerGroupName,
                                ...
                                // 重试推送
                                MqConsumerResultResp resultResp =
                                        Retryer.<MqConsumerResultResp>newInstance()
                                                .maxAttempt(pushMaxAttempt)
                                                .callable(
                                                        new Callable<MqConsumerResultResp>() {
                                                            @Override
                                                            public MqConsumerResultResp call()
                                                                    throws Exception {
                                                                MqConsumerResultResp resp =
                                                                        callServer(
                                                                                channel,
                                                                                mqMessage,
                                                                                MqConsumerResultResp
                                                                                        .class,
                                                                                invokeService,
                                                                                responseTime);
     ...
     * @since 2024.05
     */
    private <T extends MqCommonReq, R extends MqCommonResp> R callServer(
            Channel channel,
            T commonReq,
            Class<R> respClass,
