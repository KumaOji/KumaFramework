...
package com.kuma.cloud.mq.consistency.raft.impl;

import static com.kuma.cloud.mq.consistency.raft.common.NodeStatus.FOLLOWER;
import static com.kuma.cloud.mq.consistency.raft.common.NodeStatus.LEADER;
import static java.util.concurrent.TimeUnit.MILLISECONDS;

import com.kuma.cloud.mq.consistency.raft.Consensus;
import com.kuma.cloud.mq.consistency.raft.LogModule;
import com.kuma.cloud.mq.consistency.raft.Node;
import com.kuma.cloud.mq.consistency.raft.StateMachine;
import com.kuma.cloud.mq.consistency.raft.changes.ClusterMembershipChanges;
import com.kuma.cloud.mq.consistency.raft.changes.Result;
import com.kuma.cloud.mq.consistency.raft.client.ClientKVAck;
import com.kuma.cloud.mq.consistency.raft.client.ClientKVReq;
import com.kuma.cloud.mq.consistency.raft.common.NodeConfig;
import com.kuma.cloud.mq.consistency.raft.common.NodeStatus;
import com.kuma.cloud.mq.consistency.raft.common.Peer;
import com.kuma.cloud.mq.consistency.raft.common.PeerSet;
import com.kuma.cloud.mq.consistency.raft.current.RaftThreadPool;
import com.kuma.cloud.mq.consistency.raft.entity.AentryParam;
import com.kuma.cloud.mq.consistency.raft.entity.AentryResult;
import com.kuma.cloud.mq.consistency.raft.entity.Command;
import com.kuma.cloud.mq.consistency.raft.entity.LogEntry;
import com.kuma.cloud.mq.consistency.raft.entity.ReplicationFailModel;
import com.kuma.cloud.mq.consistency.raft.entity.RvoteParam;
import com.kuma.cloud.mq.consistency.raft.entity.RvoteResult;
import com.kuma.cloud.mq.consistency.raft.exception.RaftRemotingException;
import com.kuma.cloud.mq.consistency.raft.rpc.DefaultRpcClient;
import com.kuma.cloud.mq.consistency.raft.rpc.DefaultRpcServiceImpl;
import com.kuma.cloud.mq.consistency.raft.rpc.Request;
import com.kuma.cloud.mq.consistency.raft.rpc.RpcClient;
import com.kuma.cloud.mq.consistency.raft.rpc.RpcService;
import com.kuma.cloud.mq.consistency.raft.util.LongConvert;
...
public class DefaultNode implements Node, ClusterMembershipChanges {
    ...
    private LinkedBlockingQueue<ReplicationFailModel> replicationFailQueue =
     ...
     * @see NodeStatus
     */
    public volatile int status = FOLLOWER;
    ...
    public synchronized ClientKVAck handlerClientRequest( ClientKVReq request ) {
        ...
        log.warn(
                ...
                ClientKVReq.Type.value(request.getType()),
                request.getKey(),
                request.getValue());

        if (status != LEADER) {
            log.warn(
                    "I not am leader , only invoke redirect method, leader addr : {}, my addr : {}",
                    peerSet.getLeader(),
                        ...
                        try {
                            ...
                            if (result.isSuccess()) {
                                ...
                                if (result.getTerm() > currentTerm) {
                                    ...
                                    log.warn(
                                            "follower {} nextIndex not match, will reduce nextIndex and retry RPC append, nextIndex : [{}]",
                                            peer.getAddr(),
                                            nextIndex);
                                    // 重来, 直到成功.
                                }
        ...
        public void run() {
            while (running) {
                try {
                    ...
                    log.warn(
                            "replication Fail Queue Consumer take a task, will be retry replication, content detail : [{}]",
                            model.logEntry);
    ...
    class ElectionTask implements Runnable {
        ...
        public void run() {
            ...
            log.error(
                    "node {} will become CANDIDATE and start election leader, current term : [{}], LastEntry : [{}]",
                    peerSet.getSelf(),
                    currentTerm,
                    logModule.getLast());

            preElectionTime =
                    System.currentTimeMillis() + ThreadLocalRandom.current().nextInt(200) + 150;
            ...
            log.info(
                    "node {} maybe become leader , success count = {} , status : {}",
                    peerSet.getSelf(),
                    success,
                ...
                RaftThreadPool.execute(
                        () -> {
                            try {
                                ...
                                log.error(
                                        "HeartBeatTask RPC Fail, request URL : {} ",
                                        request.getUrl());
                            }
                        },
                        false);
