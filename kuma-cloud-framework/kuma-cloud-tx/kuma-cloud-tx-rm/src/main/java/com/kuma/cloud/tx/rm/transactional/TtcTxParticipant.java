...
package com.kuma.cloud.tx.rm.transactional;

import com.alibaba.fastjson2.JSONObject;
import com.kuma.cloud.tx.rm.netty.NettyClient;
import com.kuma.cloud.tx.rm.util.ApplicationContextProvider;
...
public class KmcTxParticipant {
    ...
    // 事务ID和子事务对象的映射组
    private static Map<String, KmcTx> KMC_TRANSACTIONAL_MAP = new HashMap();

    /**
     * 向事务管理者中发送一个创建事务组的命令
     */
    public static String createKmcTransactionalManagerGroup() {
        // 随机产生一个UUID作为事务组ID
     ...
     *  创建一个子事务对象
     */
    public static KmcTx createTransactional( String groupId ) {
        // 随机产生一个UUID作为子事务ID
        String transactionalId = UUID.randomUUID().toString();
        // 示例化出一个子事务对象
        KmcTx kmcTx = new KmcTx(groupId, transactionalId);
        // 将创建出的子事务对象保存到相关的变量中
        KMC_TRANSACTIONAL_MAP.put(groupId, kmcTx);
        current.set(kmcTx);
        // 对事务组数量+1
     ...
     */
    public static KmcTx addKmcTransactional( KmcTx ztp, Boolean isEnd, TransactionalType type ) {
        // 通过JSON序列化一个对象
        JSONObject sendData = new JSONObject();
        ...
        sendData.put("transactionalCount", KmcTxParticipant.getTransactionalCount());
        // 将封装好的JSON发送给事务管理者
        nettyClient.send(sendData);
    ...
    public static KmcTx getCurrent() {
        return current.get();
    }
    ...
    public static KmcTx getKmcTransactional( String groupId ) {
        return KMC_TRANSACTIONAL_MAP.get(groupId);
    }
}
