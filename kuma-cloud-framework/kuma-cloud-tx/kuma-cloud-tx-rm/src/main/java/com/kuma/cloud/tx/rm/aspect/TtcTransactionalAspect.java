...
import com.kuma.cloud.tx.rm.transactional.TransactionalType;
import com.kuma.cloud.tx.rm.transactional.KmcTx;
...
@Aspect
@Component
public class KmcTransactionalAspect implements Ordered {

    @Around("@annotation(com.kuma.cloud.tx.rm.annotation.DistributedTransactional)")
    public Integer invoke( ProceedingJoinPoint proceedingJoinPoint ) {
        ...
        if (zta.isStart()) {
            // 则向事务管理者注册一个事务组
            groupId = KmcTxParticipant.createKmcTransactionalManagerGroup();
        }
        // 否则获取当前事务所属的事务组ID
        else {
            groupId = KmcTxParticipant.getCurrentGroupId();
        }
        ...
        try {
            ...
            KmcTxParticipant.addKmcTransactional(kmcTx, zta.isEnd(), TransactionalType.commit);
            ...
            KmcTxParticipant.addKmcTransactional(kmcTx, zta.isEnd(), TransactionalType.rollback);
