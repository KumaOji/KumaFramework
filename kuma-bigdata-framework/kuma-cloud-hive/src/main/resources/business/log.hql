...
-- 系统元数据表
create external TABLE if not exists kuma_cloud_meta
(
...
stored as parquet
location "/kuma/cloud/meta";

-- 行为数据源日志表(只是描述信息项目中不能用  有$符号)
create external TABLE if not exists kuma_cloud_access_log_source
(
    project string,
    ctime   bigint,
...
location "/kuma/cloud/access/log/sources";


-- 行为数据日志转换表(和源数据合并之后的数据)
create external TABLE if not exists kuma_cloud_access_log_transform
(
    -- 基础信息
    ctime                        timestamp comment '时间--事件触发的时间',
    project                      string comment '项目--kuma-cloud-mimin-app',
...
location "/kuma/cloud/access/log/transform";


-- 行为数据日志
create external TABLE if not exists kuma_cloud_access_log_parquet
(
    -- 基础信息
    ctime                        timestamp comment '时间--事件触发的时间',
    project                      string comment '项目--kuma-cloud-mimin-app',
    ...
    partitioned by (logday string)
    stored as parquet
    location "/kuma/cloud/access/log/parquet";

-- 系统请求日志源数据表
create external TABLE if not exists kuma_cloud_request_log_source
(
...
row format serde "org.openx.data.jsonserde.JsonSerDe"
with serdepoperties ('ingore.malformed.json' = 'true')
location "/kuma/cloud/request/log/sources";

-- 系统请求日志源数据表
create external TABLE if not exists kuma_cloud_request_log_parquet
(
    trace_id          string comment '请求日志id',
    application_name  string comment '服务名称',
    request_ip        string comment '请求IP',
...
partitioned by (logday string)
stored as parquet
location "/kuma/cloud/request/log/parquet";

-- 系统日志源数据表
create external TABLE if not exists kuma_cloud_sys_log_source
(
    application_name string comment '服务名称',
    trace_id         string comment '请求日志id',
...
row format serde "org.openx.data.jsonserde.JsonSerDe"
with serdepoperties ('ingore.malformed.json' = 'true')
location "/kuma/cloud/sys/log/sources";

-- 系统日志parquet数据表
create external TABLE if not exists kuma_cloud_sys_log_parquet
(
      application_name string comment '服务名称',
      trace_id         string comment '请求日志id',
      server_ip        string comment '服务器ip',
...
partitioned by (logday string)
stored as parquet
location "/kuma/cloud/sys/log/parquet";

-- 订单日志表
create external TABLE if not exists kuma_cloud_biz_order_log_parquet
(
 ...
 partitioned by (logday string)
 stored as parquet
 location "/kuma/cloud/biz/order/log/parquet";

-- 用户登录日志表
create external TABLE if not exists kuma_cloud_biz_member_login_log_parquet
(
    member_id      bigint comment '用户id',
 ...
 partitioned by (logday string)
 stored as parquet
 location "/kuma/cloud/biz/member/login/log/parquet";
