 ...
 */
package com.kuma.cloud.hudi.util
...
object MetaUtil {
  def getMetaJson(metaType: Int): String = {
    ...
    val sql: String = "select field, field_type from `kuma-cloud-log-meta` where meta_type = " + metaType
  ...
  def getMeta: mutable.HashMap[String, String] = {
    ...
    val conn: Connection = MysqlUtil().getMysqlConn
    val statement: Statement = conn.createStatement()
    val resultSet: ResultSet = statement.executeQuery("select field, field_type from `kuma-cloud-log-meta`")
