...
package com.kuma.cloud.hudi.flink;
...
public final class HoodieFlinkQuickstart {
    ...
    public TableEnvironment getBatchTableEnv() {
        ...
        // to batch for batch execution mode(before that, both streaming and batch use streaming
        // execution mode),
        ...
        //
        // For our hudi batch pipeline upsert case, we rely on the consuming sequence for index
        // records and data records,
        // the index records must be loaded first before data records for BucketAssignFunction to
        // keep upsert semantics correct,
        // so we suggest disabling these 2 options to use streaming state-backend for batch
        // execution mode
